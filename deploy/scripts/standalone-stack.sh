...
SQL
    ...
    echo
    echo "   NOTE — the bucket LISTING gap is now closed. elitea-main serves"
    echo "   GET /artifacts/s3/{bucket}?project_id=…&list-type=2 (root-mounted,"
    echo "   not under /api/v2 — that is where the SDK asks for it), so a run"
    echo "   against a bucket holding one object now discovers that object by"
    echo "   name and reports indexed:1 on its agent_index_data_status event,"
    echo "   instead of the old vacuous \"No new documents to index.\""
    echo
    echo "   KNOWN LIMIT — the run still does not COMPLETE, because listing is"
    echo "   only the first S3-shaped call the toolkit makes. Having listed the"
    echo "   key it then fetches the bytes with"
    echo "     GET {base_url}/artifacts/s3/{bucket}/{key}?project_id=…"
    echo "   (elitea-sdk runtime/clients/client.py:1176), and THAT route does"
    echo "   not exist yet — the worker logs"
    echo "     Failed to download file '<key>': S3 error: HTTP_404"
    echo "   and the execution settles execution.failed. The object-level S3"
    echo "   verbs (GET/PUT/DELETE/HEAD on {bucket}/{key}) are all still"
    echo "   missing; only the collection-level listing is implemented."
    echo
    echo "   To seed an artifact meanwhile, use the native route:"
