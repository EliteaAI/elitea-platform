...
// PendingValidationOutbox retires expired pre-authority work and discovers
// durable, unpublished validation commands. Both operations are bounded by the
// caller's limit and use the durable store's clock. Implementations must return
// no more than limit IDs in deterministic oldest-first order and must exclude
// expired, retired and unsupported retry generations. Discovery does not claim
// ownership: duplicate discovery across publisher instances is expected and is
// handled by the dispatcher contract.
type PendingValidationOutbox interface {
	RetireNoAuthorityValidation(ctx context.Context, limit int) (int, error)
	ListPendingValidationIDs(ctx context.Context, limit int) ([]string, error)
}
...
func NewOutboxPublisher(outbox PendingValidationOutbox, dispatcher ValidationDispatchExecutor, config OutboxPublisherConfig) (*OutboxPublisher, error) {
	...
	if config.PollInterval <= 0 || config.BatchSize <= 0 || config.BatchSize > MaxOutboxPublisherBatchSize || config.MaxConcurrent <= 0 || config.MaxConcurrent > MaxOutboxPublisherConcurrency || config.MaxConcurrent > config.BatchSize || config.ReportFailure == nil {
		return nil, ErrInvalidOutboxPublisherConfig
	}
	return &OutboxPublisher{outbox: outbox, dispatcher: dispatcher, config: config}, nil
}

// Run polls immediately and then after PollInterval. Store and per-item
// dispatch failures are reported and retried in a later cycle. Context
// cancellation is terminal and retains context.Canceled/DeadlineExceeded
// identity after the active bounded worker set exits.
...
func (p *OutboxPublisher) RunOnce(ctx context.Context) error {
	...
	outboxIDs, err := p.outbox.ListPendingValidationIDs(ctx, p.config.BatchSize)
	if err != nil {
