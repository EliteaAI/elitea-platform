...
| `pytest tests/parity/test_sdk_lock.py tests/parity/test_index_ingest_current_parity.py` | Exact target worker SDK lock: distribution `0.8.53`, source revision `ccaa85f1894f34ce25074afcc232e11b406d2af1`, admitted dependency surface, plus the current Pylon single-call indexing boundary. Current Pylon SDK `0.8.30` remains the behavioral comparison baseline until the affected family harnesses pass. |
...
| `index-v2-preflight --spool-root ...` | Operator gate against the stopped version-1 release configuration: zero live v1 jobs, unsafe outstanding outboxes, unreleased claims, source stream entries, PEL entries, delivery mappings, and files in every old worker replica's private durable spool root. A retained post-authority outbox is safe only with the exact same-generation committed `SUCCEEDED`, `FAILED` or `CANCELLED` settlement matching a terminal job with `settled_at`; pre-authority/no-settlement drift still blocks. Any missing reader/group/root or dependency failure blocks. |
| `ELITEA_INDEX_V2_PREFLIGHT_SYSTEM_TEST=1 go test ./services/elitea-main/tests/system -run TestIndexV2PreflightShippedBinary` | Cross-process operator evidence: builds and executes the shipped `cmd/index-v2-preflight` binary against real PostgreSQL 16, TLS/ACL Redis, and a private spool directory. Exact settled-retained state exits `0`; nulling only outbox authority exits `1` with `outstanding_outbox=1`. |
