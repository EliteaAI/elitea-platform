...
| Current OCR fallback when attachment LLM analysis is disabled | SDK 0.8.30 Confluence PDF/image processing delegates to `langchain-community==0.4.1`, whose corresponding methods import `pytesseract`; the inherited SVG helper is callable but the current Confluence attachment router does not select it. The image pins the OCR wrapper, its wheel digest and license, Tesseract, and the deterministic DejaVu probe font |
