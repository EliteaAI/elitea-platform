"""The legacy error contract, as this sidecar reports it.

Copied from ``elitea_deepwiki.errors``, which ported it from the legacy
DeepWiki plugin; the Inventory plugin classified identically
(``methods/invoke.py::_create_error_response``) with two categories it never
emitted, so the same classifier serves both. The Go host has the authoritative
copy (``internal/spi/errors.go``) and does the classifying for a real
invocation — what crosses the socket is ``error_type``, and this module is what
decides it.

Two shapes crossed the legacy boundary and both are frozen by ADR-0022
decision 2:
...
``conformance/provider/fixtures/deepwiki/spi/errors.json`` is the test.
...
def classify(exception: BaseException) -> str:
    ...
    ``conformance/provider/fixtures/deepwiki/spi/errors.json::classifier_precedence``.
    """
