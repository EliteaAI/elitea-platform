"""The engine sidecar: the Inventory engine behind the Go host (ADR-0023 H4c).
...
``arguments`` is what the host's ``ArgumentsFor`` derived — the admitted
family, the merged parameter set and the project/toolkit ids — and the runner
is the one ``ELITEA_INVENTORY_RUNNER`` names: ``legacy`` for the engine,
``fixture`` for a stack that proves the hop without it.

The tool NAMES this sidecar admits are the ones ``tools_table`` routes, which
is the same table the Go host's descriptor advertises. The check is here as
well as there because the socket is a protocol of its own: a host that grew a
tool the engine cannot run should be refused at 400, not discovered halfway
through a stream.
...
def _known_tool(tool: str) -> bool:
    """Whether any admitted family routes this tool name."""
    return any(tool in table for table in FAMILIES.values())
...
class SidecarContext:
    ...
    asked for a stop, after terminating any tracked child process. The engine
    checkpoints between ingestion phases, which is what makes a stop land
    mid-run instead of after the whole repository has been read.
    """
...
def create_sidecar(settings, runner=None) -> FastAPI:
    ...
    app = FastAPI(title="elitea-inventory engine sidecar", docs_url=None, redoc_url=None)
    ...
    async def invoke(request: dict[str, Any]) -> StreamingResponse:
        ...
        if not isinstance(tool, str) or not _known_tool(tool):
