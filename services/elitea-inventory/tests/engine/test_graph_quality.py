"""Ported from inventory_plugin/tests/test_graph_quality.py.

The engine is copied, so its tests are copied with it: a digest proves the
bytes did not change, and these prove the behaviour those bytes implement still
holds when they run inside this package instead of inside Pylon.

Only the import bootstrap is changed — the legacy file put the plugin root on
sys.path and imported ``inventory.knowledge_graph``; here the module is
``elitea_inventory.engine.inventory.knowledge_graph``. Every assertion below is
the legacy one, unedited.

NOT ported: ``test_routing.py`` (the semantic query router needs the local
sentence-transformers model this port removed), ``test_embeddings.py`` (it
asserts the local-model embedding path, which is replaced by the gateway —
tests/unit/test_v1_scope.py covers what replaced it) and
``test_k8s_ingestion_jobs.py`` (there is no ingestion Job in v1).
"""
...
from elitea_inventory.engine.inventory.knowledge_graph import KnowledgeGraph, Citation
...
class TestFuzzyIdResolution:
    ...
    def _make_resolve_id(entities):
        ...
        from elitea_inventory.engine.inventory.extractors import RelationExtractor
...
class TestEntityTypePriority:
    ...
    def test_priority_constants(self):
        ...
        from elitea_inventory.engine.inventory.ingestion import _entity_type_priority
    ...
    def test_build_entity_by_name_class_wins_over_import(self):
        ...
        from elitea_inventory.engine.inventory.ingestion import _build_entity_by_name
        entities = [
    ...
    def test_build_entity_by_name_ids_class_wins(self):
        ...
        from elitea_inventory.engine.inventory.ingestion import _build_entity_by_name_ids
        entities = [
