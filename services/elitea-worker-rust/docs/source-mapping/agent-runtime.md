...
| `libs/proto/elitea/runtime/v1/{control,output}.proto` | Claim, begin, authorize, lease, desired state, settlement, ordered output stream | `src/protocol/output.rs`, `src/transport/{control_grpc,output_session,output_grpc}.rs`, planned `src/protocol/control.rs` | None | Python progress/success/cancellation frame goldens; control message-boundary and ACK/credit/replay state corpora | Partial: deterministic framing plus one-attempt control/output transports are implemented; semantic control authorization, settlement ordering and reconnect coordination are planned |
...
decisions in subsequent slices.
