...
| Python worker `execution/delivery.py::{_begin_execution,_authorize_invocation}` and Main durable claim state | Distinguish restart-safe preparation from the one durable native-ADK submission fence; retain output authority for authenticated semantic failures while treating transport loss as unknown-effect no-ACK recovery | `src/protocol/control.rs::{BeginAgentExecution,LeaseStartingAgentExecution,LeaseMonitoredAgentExecution,InvocationAuthorizationCandidate,InvocationAuthorizationDecision,AuthorizedAgentInvocation,InvocationAuthorizationTerminal,InvocationAuthorizationUnknown}`, `src/execution/agent_preparation.rs::PreparedAgentAuthorizationPayload` | The later ADK entry consumes the inseparable submission/output/prepared-run state exactly once | Exact Python decision vectors, application/ad-hoc payload carry-through, all closed authorization outcomes, server-text redaction, mixed-response rejection and owned typestate transitions | Implemented semantic boundary. `STARTED_NOW` is not an invocation token; business input remains inaccessible until the immediate supervised lease poll activates it. Preparation atomically binds the request, verified command, Redis delivery, capacity reservation and supervised lease to its claim before the authorization call. Only `AUTHORIZED_NOW` carries that complete payload into a one-shot submission/output state. `ALREADY_AUTHORIZED`, authenticated rejection and malformed authenticated response retain terminal-only authority plus the matching payload; transport uncertainty grants neither terminal nor submission authority. No API can authorize claim A while submitting claim B's prepared request |
...
| Python worker `execution/delivery.py::_prepare_frame_settlement` and `control_grpc.py::ExecutionControlClient.prepare_settlement` | Fresh or restored terminal bound ACK and spool retirement precede deterministic PrepareSettlement; Redis retirement follows the validated receipt | `src/transport/output_grpc.rs::{OutputGrpcSession::send_terminal,PreparedOutputSpool::replay_terminal}`, `src/protocol/control.rs`, `src/transport/redis_commands.rs`, `src/execution/output_delivery.rs::{publish_pre_invocation_terminal,recover_accepted_terminal}` | None | Fresh bound-ACK/proposal-digest/receipt and restricted exact-delivery retirement component tests; restored-terminal ACK/missing-ACK proof; fresh and recovered coordinator `replay -> settlement -> Redis` order and failure-edge tests | Implemented for accepted local-terminal recovery and fresh pre-invocation failure terminals behind internal capability gates. Fresh publication and byte-exact restored replay create the same consuming settlement proof only after the bound ACK has durably retired the local spool entry. Semantic control prepares the exact settlement, and the restricted retirement boundary consumes only the matching proof/delivery. Settlement failure never reaches Redis; Redis failure cannot regenerate or replay the already-ACKed terminal. Authorized invocation results/progress, real TLS/ACL Redis and cross-process ACK-loss component tests remain gates |
| Python worker `execution/delivery.py::AgentExecutionDeliveryProcessor` and inherited `_process_admitted` | Shared application/ad-hoc verification, all ten claim dispositions, recovery/settlement routing and eventual authorize-once execution | `src/execution/{agent_delivery,output_delivery,agent_preparation}.rs`, planned authorize/run/output stages | Runner invoked behind the effect fence | `tests/agent_delivery_route_contract.rs` plus `src/execution/{output_delivery_tests,agent_preparation_tests}.rs`: both entrypoints, all ten claim classes, accepted-terminal replay/settlement/retirement ordering, spool-first ownership, shared fresh preparation and pre-invocation failure retirement | Partial: the shared router, no-network spool preflight, accepted local-terminal recovery coordinator, fresh preparation through a fully bound typed request, and Stop/input/deadline terminal retirement are implemented. This is not whole-delivery admission: Redis intake/reclaim, running-progress recovery, authorization outcome composition, ADK execution and fresh progress/result publication remain coordinator gates |
