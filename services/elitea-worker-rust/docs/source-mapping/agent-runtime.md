...
| Python worker `execution/delivery.py::{_begin_execution,_authorize_invocation}` and Main durable claim state | Distinguish restart-safe preparation from the one durable native-ADK submission fence; retain output authority for authenticated semantic failures while treating transport loss as unknown-effect no-ACK recovery | `src/protocol/control.rs::{BeginAgentExecution,LeaseStartingAgentExecution,LeaseMonitoredAgentExecution,InvocationAuthorizationCandidate,InvocationAuthorizationPayload,InvocationAuthorizationDecision,AuthorizedAgentInvocation,InvocationAuthorizationUnknown}`, `src/execution/agent_preparation.rs::{PreparedAgentAuthorizationPayload,AgentFailureTerminal}` | The later ADK entry consumes the inseparable submission/output/prepared-run state exactly once | Exact Python decision vectors, application/ad-hoc payload carry-through, all closed authorization outcomes, server-text redaction, mixed-response rejection, direct authorization-terminal publication and owned typestate transitions | Implemented semantic boundary. `STARTED_NOW` is not an invocation token; business input remains inaccessible until the immediate supervised lease poll activates it. Preparation atomically binds the request, verified command, Redis delivery, capacity reservation and supervised lease to its claim before the authorization call. Only `AUTHORIZED_NOW` carries that complete payload into a one-shot submission/output state. `ALREADY_AUTHORIZED`, authenticated rejection and malformed authenticated response consume the prepared payload and exact output claim together into one terminal-only state while dropping all request authority; transport uncertainty retains the payload behind no-ACK authority and grants neither terminal nor submission authority. No tuple/map escape hatch can authorize claim A while publishing or submitting claim B |
...
| --- | --- | --- | --- | --- | --- |
| Python worker `execution/delivery.py::_execute_resolved` and `handlers/agent.py::AgentExecutionHandler.execute` | After durable authorization, select exactly one application/ad-hoc runtime entry and stream its semantic events | planned `src/execution/agent_invocation.rs`, `src/agents/assembly.rs`; dependency pin in `Cargo.toml` | Exact ADK-Rust `2.0.0` `LlmAgent`, `Runner`, session, model and tool/graph façades behind the Elitea authority boundary | `tests/adk_native_primitives.rs`: credential-free model -> agent -> session -> runner, `FunctionTool`/`BasicToolset`, bounded graph fork/join | Foundation only: the exact native substrate compiles and executes. It is not connected to `AuthorizedAgentInvocation`, production providers, output delivery or capability registration |
