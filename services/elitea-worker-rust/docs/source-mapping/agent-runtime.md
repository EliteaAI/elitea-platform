...
| --- | --- | --- | --- | --- | --- |
| Python worker `execution/delivery.py::_execute_resolved` and `handlers/agent.py::AgentExecutionHandler.execute` | After durable authorization, select exactly one application/ad-hoc runtime entry and stream its semantic events | planned `src/execution/agent_invocation.rs`, `src/agents/assembly.rs`; `src/execution/agent_preparation.rs::AuthorizedAgentRun`; dependency pin in `Cargo.toml` | Exact ADK-Rust `2.0.0` `LlmAgent`, `Runner`, session, model and tool/graph façades behind the Elitea authority boundary | `tests/adk_native_primitives.rs`: credential-free model -> agent -> session -> runner, `FunctionTool`/`BasicToolset`, bounded graph fork/join | Foundation only: the exact native substrate compiles and executes. Authorization now produces one inseparable run with the request, output, lease, reservation and one-use permit, but it is not yet connected to production providers, ADK event projection, output delivery or capability registration |
