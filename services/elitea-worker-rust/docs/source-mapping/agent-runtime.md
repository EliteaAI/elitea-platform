...
| Python worker `execution/delivery.py::AgentExecutionDeliveryProcessor` and inherited `_process_admitted` | Shared application/ad-hoc verification, all ten claim dispositions, recovery/settlement routing and eventual authorize-once execution | `src/execution/{agent_delivery,output_delivery,agent_preparation}.rs`, planned supervised authorize/run/output stages | Runner invoked behind the effect fence | `tests/agent_delivery_route_contract.rs` plus `src/execution/{output_delivery_tests,agent_preparation_tests}.rs`: both entrypoints, all ten claim classes, accepted-terminal replay/settlement/retirement ordering, spool-first ownership, shared fresh preparation and pre-invocation/authorization failure retirement | Partial: the shared router, no-network spool preflight, accepted local-terminal recovery coordinator, fresh preparation through a fully bound typed request, and Stop/input/deadline/authenticated-authorization terminal retirement are implemented. This is not whole-delivery admission: Redis intake/reclaim, running-progress recovery, supervisor-owned authorization, ADK execution and fresh progress/result publication remain coordinator gates |
| Python worker `execution/sync_executor.py::{BoundedAdmissionGate,BoundedSyncExecutor.reserve,SyncExecutorReservation}` and `execution/delivery.py::_reserve_execution_capacity` | Bound running plus queued invocation work; reserve before `BeginExecution`; cancel/timeout/worker-drain without Begin/input effects; release unused capacity on every pre-submit path | `src/execution/invocation_admission.rs::{InvocationAdmission,InvocationReservation}` and `src/execution/agent_preparation.rs` | Native ADK execution will consume the opaque slot; no Python thread-pool compatibility layer is reproduced | `src/execution/{invocation_admission_tests,agent_preparation_tests}.rs`: exact bounds, capacity, cancellation, timeout/drain, saturation-before-Begin and unused release | Implemented through preparation. Tokio's fair bounded semaphore and explicit accepting state produce stable, data-free retry/no-ACK saturation or drain outcomes. The reservation is acquired before Begin and remains inside `PreparedAgentInvocation`; the later runner must retain it until the real non-cancellable ADK call ends |
| Python worker `execution/sync_executor.py::BoundedSyncExecutor.run`, `execution/supervisor.py::ExecutionSupervisor` and process-drain handling in `serve.py` | Once submitted, real invocation work outlives a cancelled result waiter, retains its capacity slot until it exits and is joined during worker drain; process shutdown is not business Stop | `src/execution/invocation_supervisor.rs::{InvocationSupervisor,SupervisedInvocation}`, `src/diagnostics.rs` | Tokio-owned task group around the future returned by the later invocation-scoped ADK runner | `src/execution/invocation_supervisor_tests.rs`: dropped waiter, cancellation-safe drain, stop/submit race, admission wake, foreign-pool rejection and redacted contained panic/resource release | Internal foundation implemented. Submission is a synchronous ownership transfer with no cancellation await gap, and every task must consume a reservation from the supervisor's exact bounded admission pool. Stop closes that pool; a losing submission receives its original reservation and potentially authority-bearing future intact. The module is deliberately not exported or capability-registered: the full coordinator must own the job before `AuthorizeInvocation`, carry no-network spool preflight, retain lease/output authority through terminal ACK, settlement and Redis retirement, and use invocation-scoped rather than session-wide ADK cancellation |
