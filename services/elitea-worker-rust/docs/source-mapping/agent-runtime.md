...
| Python worker `execution/delivery.py::AgentExecutionDeliveryProcessor` and inherited `_process_admitted` | Shared application/ad-hoc verification, all ten claim dispositions, recovery/settlement routing and eventual authorize-once execution | `src/execution/agent_delivery.rs::AgentDeliveryRouter`, planned integrated coordinator stages | Runner invoked behind the effect fence | `tests/agent_delivery_route_contract.rs`: application/ad-hoc fresh routing, all ten disposition classes and exact terminal Claim -> optional PrepareSettlement -> Redis retirement ordering; later ACK-loss/cancellation cases remain planned | Partial: the shared pre-execution router is implemented, and terminal recovery cannot return success before exact Redis retirement. This is not whole-delivery admission: Redis intake/reclaim, capacity reservation, spool recovery, lease supervision, input materialization, Begin/Authorize, ADK invocation and terminal publication remain coordinator gates |
| Python worker `execution/sync_executor.py::{BoundedAdmissionGate,BoundedSyncExecutor.reserve,SyncExecutorReservation}` and `execution/delivery.py::_reserve_execution_capacity` | Bound running plus queued invocation work; reserve before `BeginExecution`; cancel/timeout/worker-drain without Claim-side effects; release unused capacity on every pre-submit path | `src/execution/invocation_admission.rs::{InvocationAdmission,InvocationReservation}` | Native ADK execution will consume the opaque slot; no Python thread-pool compatibility layer is reproduced | `src/execution/invocation_admission_tests.rs`: exact configuration bounds, running-plus-queued capacity, unused release, cancellation withdrawal, timeout and drain wakeup/ownership | Implemented admission primitive. It uses Tokio's fair bounded semaphore and an explicit accepting state; saturated/draining errors are stable, data-free and retryable. Existing reservations remain owned during drain. The fresh coordinator still must acquire before Begin and retain the reservation through real ADK completion; this slice intentionally exposes neither a raw semaphore permit nor a standalone invocation runner |
