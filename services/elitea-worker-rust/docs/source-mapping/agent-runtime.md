...
| Python worker `execution/delivery.py::_process_admitted` output-spool inspection before `_reserve_execution_capacity` and `_begin_execution` | Open and exclusively own the execution-bound encrypted spool before capacity, Begin, input or authorization; route any pending frame to recovery instead of fresh execution | `src/execution/output_delivery.rs::{AgentOutputPreflight,EmptyAgentOutput,PendingAgentOutput}` plus `src/spool.rs` and `src/transport/output_grpc.rs::PreparedOutputSpool` | None | `src/execution/output_delivery_tests.rs`: empty exclusive ownership/reopen, producer and workload-session substitution before directory creation, sole claim-bound pending progress and terminal frames, foreign-fence rejection; every preparation test now constructs the required empty-spool proof | Implemented ordering boundary. Preflight runs synchronous filesystem/crypto work on Tokio's blocking pool, binds both output metadata identities to the sealed accepted claim, retains the exclusive directory lock in a non-cloneable typestate and exposes only stable data-free errors. `prepare_fresh_agent_invocation` accepts only `EmptyAgentOutput`, so pending output cannot reach reserve or Begin. Current-profile preflight permits exactly one unacknowledged NodeEvent or terminal-shaped agent frame; exact payload/proposal semantic recovery and endpoint reconnect remain the next output-coordinator slice |
