...
| Python worker `execution/delivery.py::{_begin_execution,_authorize_invocation}` and Main durable claim state | Distinguish restart-safe preparation from the one durable SDK submission fence | `src/protocol/control.rs::{BeginAgentExecution,LeaseStartingAgentExecution,LeaseMonitoredAgentExecution,InvocationAuthorizationDecision,InvocationPermit}` | ADK entry will consume the opaque permit later | Exact Python decision vectors, mixed response and owned typestate transitions | Implemented semantic boundary. `STARTED_NOW` is not treated as an invocation token; business input remains inaccessible until the immediate supervised lease poll activates it, an owned authenticated operation consumes preparation, and only `AUTHORIZED_NOW` mints one private-field, non-`Clone` permit |
