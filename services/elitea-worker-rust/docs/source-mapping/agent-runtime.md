...
| Python worker `execution/delivery.py::_process_admitted`, `_await_with_lease_state`, agent `_resolve_inputs` and `_execute_resolved` pre-authorization half | Reserve before Begin; require `STARTED_NOW`; complete immediate Renew -> Observe before input; race materialization with Stop/fatal lease; validate canonical application/ad-hoc request; sample inclusive deadline after full request construction; stop before `AuthorizeInvocation` | `src/execution/agent_preparation.rs::{prepare_fresh_agent_invocation,PreparedAgentInvocation,PreInvocationTerminal}` | None before authorization; the later native ADK runner consumes the prepared request and opaque reservation | `src/execution/agent_preparation_tests.rs`: shared application/ad-hoc ordering, saturation, Begin replay, immediate/periodic Stop, cancelled input future, input failure plus final lease poll, fatal-over-terminal, dependency mapping, inclusive and request-validation deadline crossings | Implemented fresh preparation boundary. Stop, input and deadline causes retain exact pre-invocation output authority for canonical terminal publication; lease/control/clock failures remain typed no-ACK errors. `PreparedAgentInvocation` keeps the exact delivery, verified command, live lease, validated request and capacity slot private, so callers cannot authorize or submit by copying raw fence data |
...
| Python worker `execution/delivery.py::AgentExecutionDeliveryProcessor` and inherited `_process_admitted` | Shared application/ad-hoc verification, all ten claim dispositions, recovery/settlement routing and eventual authorize-once execution | `src/execution/{agent_delivery,agent_preparation}.rs`, planned authorize/run/output stages | Runner invoked behind the effect fence | `tests/agent_delivery_route_contract.rs` plus `src/execution/agent_preparation_tests.rs`: both entrypoints, all ten claim classes, terminal recovery ordering and the shared fresh preparation trace | Partial: the shared router and fresh preparation through a fully bound typed request are implemented. This is not whole-delivery admission: Redis intake/reclaim, encrypted-spool inspection/recovery, invocation authorization, ADK execution, progress/terminal publication and final retirement remain coordinator gates |
