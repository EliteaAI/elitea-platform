...
| Current gap: Python `elitea_sdk/runtime/langchain/langraph_agent.py::create_graph` has no graph-parallel node | Bounded true fork/join graph node, deterministic reducer/error policy and restart-safe join | `src/agents/graph/parallel.rs`, durable ledger in graph checkpoint state | ADK `StateGraph` fan-out and `CompiledGraph::with_max_concurrency`; Elitea persists deferred arrivals because ADK 2.0.0 keeps them only in process memory | Concurrency, deterministic join, crash between unequal arrivals, branch HITL, failure and cancellation tests | Intentional improvement; planned next. YAML v1 supports only `wait: all`; `one`/`many` are rejected until a durable early-release scheduler exists |
...
| Python worker `agents/checkpoint.py::CurrentAgentCheckpointFactory`, `agents/sdk_adapter.py::{_execution_memory,_configure_authorization_checkpoint_resume}` and legacy Centry `utils/agent_execution_common.py::{setup_memory,create_memory_saver,configure_checkpoint_resume}` | Select current checkpoint storage, scope one execution memory context and configure initial/resume input | `src/state/postgres_checkpointer.rs`, migration `services/elitea-main/migrations/shared/0064_agent_graph_checkpoints.sql`, planned `src/agents/session.rs` composition | Native ADK 2.0.0 `Checkpointer` with the bundled SQLite backend as behavioral baseline; Elitea owns tenant/project/thread/graph-definition scoping and live claim/fence writer control | Six bounded/canonical-serialization/error unit tests plus real PostgreSQL 18 complete-field/nanosecond/arbitrary-number/exact-retry/save-order/wrong-thread/definition-isolation/release-race/newer-claim/prune/delete/pool-reuse component story; migration static test | Storage primitive implemented for a fresh `adk-graph.2.0.0.v1` lineage. No LangGraph blob/pickle row compatibility is intended. The caller supplies one shared bounded `PgPool`; each operation uses and releases a transaction. Invocation composition, immutable graph-digest derivation, restricted DB grants, Linux/pooler/load proof and Python-lineage cutover remain gates |
