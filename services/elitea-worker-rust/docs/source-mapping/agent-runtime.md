...
| Python worker `transport/redis_commands.py::{RedisCommandDelivery,RedisCommandConsumer.ack_after_settlement,_is_confirmed_retirement}` and `transport/redis_asyncio.py::RedisAsyncioControlClient.retire_delivery` | Preserve the exact bounded signed-envelope entry and expose only atomic owner-bound `XACK + XDEL + delivery-index HDEL` after durable terminal authority | `src/transport/redis_commands.rs::{RedisCommandDelivery,RedisCommandRetirer,RedisRetirementClient}` | None | `tests/redis_command_retirement_contract.rs`: duplicate/unknown/entry-size corpus, exact and valid reordered outer-envelope bytes, same-identity changed-intent/cross-command authority substitution, and both confirmed Lua result shapes | Partial: strict post-RESP decode and a consumed exact-authenticated-envelope/identity/outbox-bound retirement proof are implemented. No arbitrary ACK/delete/publish method exists. The real TLS/ACL Redis RESP client, restricted Lua wiring, read/reclaim and PEL heartbeat remain planned |
...
- The restricted Redis retirement adapter now consumes opaque terminal proof
  ...
  exact signed-envelope binding. A real TLS/ACL Redis client and the restricted
  Lua script still need service/component proof; no production Redis effect is
  claimed by the injected-client contract tests.
