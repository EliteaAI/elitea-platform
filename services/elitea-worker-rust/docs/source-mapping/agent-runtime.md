...
| Python worker `execution/delivery.py::_prepare_frame_settlement` and `control_grpc.py::ExecutionControlClient.prepare_settlement` | Terminal bound ACK and spool retirement precede deterministic PrepareSettlement; Redis retirement follows the validated receipt | `src/transport/output_grpc.rs`, `src/protocol/control.rs`, `src/transport/redis_commands.rs`, planned `src/execution/agent_delivery.rs` | None | Bound-ACK/proposal-digest/receipt and restricted exact-delivery retirement component tests; recovery and end-to-end Redis-order system tests planned | Partial: output ACK creates a consuming proof, semantic control prepares the exact settlement, and the restricted retirement boundary consumes only the matching proof/delivery. Cross-step delivery coordination and the real Redis effect remain planned |
