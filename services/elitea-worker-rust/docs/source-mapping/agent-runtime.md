...
| `libs/proto/elitea/runtime/v1/{control,output}.proto` | Claim, begin, authorize, lease, desired state, settlement, ordered output stream | `src/protocol/{control,output}.rs`, `src/transport/{control_grpc,output_session,output_grpc}.rs` | None | Python progress/success/cancellation/control goldens; claim/manifest mutation, control sum-type, lease, message-boundary and ACK/credit/replay state corpora | Partial: fresh agent claim authority, begin/authorize decisions, non-clone invocation permit, lease/desired-state semantics, settlement receipt validation and one-attempt control/output transports are implemented; recovery dispositions, settlement ordering and shared delivery coordination remain planned |
...
| Python worker `execution/delivery.py::{_prepare_frame_settlement,_settlement_receipt}` | Only a bound durable terminal ACK can create the exact deterministic settlement request and validated receipt | `src/transport/output_grpc.rs::{OutputGrpcSession::send_terminal,DurablyAckedTerminal}`, `src/protocol/control.rs::{AgentControlClient::prepare_agent_settlement,SettlementReceipt}` | None | Bound ACK to consumed settlement proof component test; Python outcome/receipt vector | Implemented fresh terminal settlement authority. Raw success protobufs cannot construct receipts; recovered server-supplied settlement material remains planned |
