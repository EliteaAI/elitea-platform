...
| Python worker `execution/delivery.py::{_ClaimLeaseMonitor,_await_with_lease_state}` | Exact-fence deterministic renewal keys; sequential Renew -> Observe; post-observation wall-clock margin; monotonic skip cadence; sticky Stop while renewal continues; fatal-over-cancellation priority; cancellation of pre-SDK async work only | `src/execution/agent_lease.rs::{ClaimLeaseMonitor,ClaimLeaseActivation,ClaimLeaseError}`, crate-private control operations over `src/protocol/control.rs::ClaimLeaseHandle` | None | `src/execution/agent_lease_tests.rs`: exact call order/key sequencing, 1ms/10s v1 cadence bounds, post-observation margin, cancellation latch, later fatal priority, DRAINING policy, missed-tick skip, pre-invocation cancellation and stalled-RPC close; `tests/agent_control_contract.rs` checks the exact Python renewal-key vector and mandatory activation typestate | Implemented supervised lease lifecycle. The actor owns the only claim-and-handle package, uses bounded cancellation-safe command/state channels, and keeps renewing after user Stop so a non-cancellable synchronous invocation can retain its fence. Public configuration accepts only whole-millisecond intervals from 1ms through the deployed 10s maximum; only `ClaimLeaseActivation::Active` exposes business inputs. Main does not currently deduplicate renewal keys, so the generated key is correlation/idempotency intent rather than claimed server replay protection. The integrated coordinator still must retain this monitor through invocation, terminal ACK and settlement |
...
| Python worker `execution/delivery.py::_prepare_frame_settlement` and `control_grpc.py::ExecutionControlClient.prepare_settlement` | Fresh or restored terminal bound ACK and spool retirement precede deterministic PrepareSettlement; Redis retirement follows the validated receipt | `src/transport/output_grpc.rs::{OutputGrpcSession::send_terminal,PreparedOutputSpool::replay_terminal}`, `src/protocol/control.rs`, `src/transport/redis_commands.rs`, planned `src/execution/agent_delivery.rs` | None | Fresh bound-ACK/proposal-digest/receipt and restricted exact-delivery retirement component tests; restored-terminal ACK proof plus missing-ACK exact-spool retention; recovery and end-to-end Redis-order system tests planned | Partial: both fresh publication and byte-exact restored replay create the same consuming settlement proof only after the bound ACK has durably retired the local spool entry. Semantic control prepares the exact settlement, and the restricted retirement boundary consumes only the matching proof/delivery. The restored replay entry remains crate-private and unregistered until the owned bounded-reconnect coordinator composes it with settlement and the real Redis effect |
| Python worker `execution/delivery.py::AgentExecutionDeliveryProcessor` and inherited `_process_admitted` | Shared application/ad-hoc verification, all ten claim dispositions, recovery/settlement routing and eventual authorize-once execution | `src/execution/{agent_delivery,output_delivery,agent_preparation}.rs`, planned authorize/run/output stages | Runner invoked behind the effect fence | `tests/agent_delivery_route_contract.rs` plus `src/execution/{output_delivery_tests,agent_preparation_tests}.rs`: both entrypoints, all ten claim classes, terminal recovery ordering, spool-first ownership and the shared fresh preparation trace | Partial: the shared router, no-network spool preflight and fresh preparation through a fully bound typed request are implemented. This is not whole-delivery admission: Redis intake/reclaim, exact pending-output recovery, invocation authorization, ADK execution, progress/terminal publication and final retirement remain coordinator gates |
