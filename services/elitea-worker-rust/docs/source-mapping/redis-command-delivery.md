...
| Python `serve.py::WorkerServeLoop` | Fair new/reclaim intake, bounded queue plus active jobs, heartbeat both sets, reclaim after loss, stop admission then drain | Planned Rust serve-loop owner over `RedisStreamsClient`, `AgentInvocationSupervisor` and the shared app/ad-hoc delivery router | Not yet implemented. This is the next Redis composition slice after authorized event/output lifecycle, not a reason to put Redis inside ADK or the event projector |
