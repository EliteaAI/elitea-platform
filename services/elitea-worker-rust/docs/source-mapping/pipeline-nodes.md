...
Inner model/tool events currently execute inside the graph node and are not yet
forwarded through the outer pipeline event stream, so the browser sees the
confirmation and final node result but not every inner progress event. Approved
effectful calls also remain closed for production until durable intent/outcome
receipts can distinguish a crash before dispatch from an unknown result after
dispatch. MCP OAuth/on-demand authorization is a separate interrupt contract.
