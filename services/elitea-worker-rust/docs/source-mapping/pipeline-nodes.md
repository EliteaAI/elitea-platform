...
| UI `AgentNode.jsx`; SDK `langraph_agent.py` Agent branch plus `runtime/tools/application.py::Application` | Invoke one selected saved entity by mapping exactly one `task`, append its final response to `messages`, and copy that response to declared outputs. The selected `application` entity can resolve to either a direct agent or a saved pipeline | `src/agents/graph/{application,compiler}.rs`, `src/agents/{application_tools,pipeline}.rs`, `src/transport/platform_client.rs` | ADK `AgentTool` for a direct saved agent; native `SubgraphNode` is the required follow-up for a saved pipeline so inner interrupts remain graph interrupts | Partial capability-disabled implementation. The compiler admits the active strict YAML, fixed/variable/f-string task mappings, bounded output projection, exact frozen alias and only `agent`/`pipeline` participant kinds. Production assembly currently resolves variable-free direct saved agents through the existing claim-bound application/version loader and native `AgentTool`; four node fixtures plus pipeline admission prove mapping, exact response shape, common Runner/checkpoint completion and fail-closed authority. A selected saved pipeline is admitted as a valid participant kind but still fails during child assembly: wrapping its `GraphAgent` in `AgentTool` would turn an inner interrupt into an ordinary event, so the next slice must compile it as an isolated, checkpointed ADK `SubgraphNode`. Child variable schemas, the missing private immutable Main child-version endpoint, nested HITL/MCP-auth/effect receipts and static `interrupt_after` remain explicit gates. |
