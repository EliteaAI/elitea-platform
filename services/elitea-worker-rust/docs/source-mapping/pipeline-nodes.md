...
turn after native node events. Raw incremental tool-progress chunks and a full
breadcrumb container for an LLM inside a saved-pipeline child remain separate
gaps. Approved effectful calls also remain closed for production until durable
intent/outcome receipts can distinguish a crash before dispatch from an unknown
result after dispatch. MCP OAuth/on-demand authorization is a separate interrupt
contract.
