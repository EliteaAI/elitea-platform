...
| Platform Python worker `handlers/indexing.py::IndexIngestDeliveryProcessor` and progress callback | Claim-bound SDK execution and bounded `NodeEventV1` progress | `src/indexing/delivery.rs`, `src/indexing/events.rs`, shared `src/protocol/node_event.rs` | Delivery, progress ordering, terminal/error and cancellation component tests | Planned. ADK graph/tool custom events are candidate semantic inputs only; raw JSON and broadcast delivery never replace the durable output ACK path |
...
| Centry indexing callbacks and event helpers | Thinking/progress/error/terminal payloads | `src/indexing/events.rs`, shared `src/protocol/node_event.rs` | Ordered current UI/event corpus | Planned |
