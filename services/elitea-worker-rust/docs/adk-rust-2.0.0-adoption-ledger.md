...
| Tools/toolsets | `Tool`, `Toolset`, `BasicToolset`, typed tool macro | Adopt and wrap | Elitea owns saved configuration, selected-tool identity, grants, sensitive policy, absolute deadline, bounded concurrency and result projection. The capability-disabled GitHub reference family now parses the claim-materialized SDK shape, shares one invocation-scoped pooled client between its real probe and an explicitly selected `get_me`/`list_branches_in_repo` ADK toolset, and rejects the unported catalog rather than weakening empty-selection semantics. Main retains the public check API/auth/revision/audit contract; live dispatch, the remaining 42 tools, App installation auth, HITL and indexing remain gates |
