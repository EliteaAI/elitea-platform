...
| Event stream | `EventStream`, `Event`, `Content`, `Part` | Adopt as internal semantic input, not as the browser/output contract | The closed ordinary-text projector maps bounded root-model text/thinking events to current `NodeEventV1` shapes and drains through EOS. Elitea's claim-bound output cursor owns exact fence/sequence binding and advances only after an opaque exact-frame ACK proof; an exact Main cancellation/deadline rejection produces distinct same-sequence replacement authority after the required protocol bootstrap. Restored progress replay now has an owned transport task which stores the exact ACK or rejection proof before notifying its retryable waiter, and explicit close keeps its join owner across cancellation. Owner Drop abandons local continuation to Main claim/handoff recovery, so the sealed capability-disabled publisher must keep task and cursor inseparable. That bounded publisher, sanitization, durable ADK composition and tool/HITL/MCP/application completion projection remain Elitea-owned gates |
