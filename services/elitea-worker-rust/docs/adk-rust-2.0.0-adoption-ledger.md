...
| --- | --- | --- | --- |
| Agent and runner | `LlmAgent`, `GraphAgent`, `Runner`, `RunConfig`, `Event`, `AgentTool` | Adopt behind the sealed one-shot invocation state | Main claim/authorize fence, supervised lifetime, progress/terminal output and Redis retirement remain Elitea-owned. Application `agent_type=agent` and ad-hoc build one direct `LlmAgent`; configured native toolsets and variable-free saved direct children join that loop through `AgentTool`. Stored `agent_type=pipeline` builds a graph agent through the same exclusive Runner/SessionService entrypoint and adds a Checkpointer rather than wrapping direct agents in fake graphs. Runtime/admin policy projects `require_tool_confirmation` on each owning `LlmAgent`; pipeline HITL remains graph-owned. Capability-disabled direct continuation restores the exact persisted call without provider replanning, while pipeline continuation reconstructs the graph around one exact private checkpoint decision. Production routing, approved-effect ownership, MCP authorization, general graph nodes and active context management remain closed |
...
| Graph | `StateGraph`, custom `Node`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The bounded `parallel` core and an initial complete-document compiler/assembler for strict stored `type: hitl` nodes are implemented. The authorized application selector compiles complete frozen `agent_type=pipeline` YAML before any model, tool or state backend is constructed. Pipeline HITL uses ADK dynamic checkpoint/resume, exact definition digests, one-use private decisions and fail-closed routes; it is not sensitive-tool confirmation and carries no tool-call ID. `EliteaGraphAgent` repairs root/invocation identity, emits one fixed state-free terminal marker and uses the same Runner/SessionService entrypoint as direct agents with an additional Checkpointer. The compiler rejects configured tools, every unimplemented node family and static breakpoint before graph construction. Elitea still owns production routing, continuation-suffix recovery, value-producing output selection, general node/edge/reducer policy, static breakpoint semantics and durable fan-in extension |
