...
| Graph | `StateGraph`, custom `Node`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The bounded `parallel` core and an initial complete-document compiler for strict stored `type: hitl` nodes are implemented. Pipeline HITL uses ADK dynamic checkpoint/resume, exact definition digests, one-use private decisions and fail-closed routes; `EliteaGraphAgent` repairs root/invocation identity before browser projection. The compiler rejects every unimplemented node family and static breakpoint before graph construction. Elitea still owns authorized PostgreSQL assembly, general node/edge/reducer policy, static breakpoint semantics and durable fan-in extension |
