...
| Graph | `StateGraph`, custom `Node`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The bounded `parallel` core and a complete-document compiler/assembler for strict stored `type: hitl` plus `type: state_modifier` nodes are implemented. The authorized application selector compiles complete frozen `agent_type=pipeline` YAML before any model, tool or state backend is constructed. Pipeline HITL uses ADK dynamic checkpoint/resume, exact definition digests, one-use private decisions and fail-closed routes; it is not sensitive-tool confirmation and carries no tool-call ID. The state modifier uses fuel- and output-bounded MiniJinja with the four current SDK filters, typed state defaults/output and declared cleaning. `EliteaGraphAgent` repairs root/invocation identity, emits either one fixed state-free marker or one explicit terminal result through the normal browser model lifecycle, and uses the same Runner/SessionService entrypoint as direct agents with an additional Checkpointer. The shared native router delegates the graph branch without try/fallback semantics. The next LLM-node slice must instantiate a nested ADK `LlmAgent` with only the UI-connected names resolved from claim-authorized toolsets; it must not inherit ad-hoc scope. The compiler rejects every unimplemented node family and static breakpoint before graph construction. Elitea still owns production bootstrap/Redis registration, continuation-suffix recovery, remaining active-node semantics, static breakpoints and durable fan-in extension |
