...
| Observability | `tracing` instrumentation below ADK plus Elitea lifecycle spans | Wrap at orchestration/tool boundaries | The binary installs a crate-only level-filtered subscriber. Authenticated prepare/lifecycle, provider-neutral assembly, configured tool calls, nested `AgentTool` calls and pipeline LLM nodes carry safe correlation, phase, outcome, duration and stable code fields. The LLM-node span records only node identity, selected-tool counts, structured mode and stage; prompts, history, arguments, results, URLs, credentials and provider bodies are excluded. OTLP/metrics export and deployed retention/cardinality policy remain the next local-platform gate |
...
| Graph | `StateGraph`, `StateSchema`, native reducers, custom `Node`, nested `LlmAgent`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The bounded `parallel` core and a complete-document compiler/assembler for strict stored `type: hitl`, `type: state_modifier` and `type: llm` nodes are implemented. The authorized application selector compiles complete frozen `agent_type=pipeline` YAML before any model, tool or state backend is constructed. Pipeline HITL uses ADK dynamic checkpoint/resume, exact definition digests, one-use private decisions and fail-closed routes; it is not sensitive-tool confirmation and carries no tool-call ID. Runtime-owned state uses ADK reducers: the bounded JSON `messages` business channel appends, HITL decisions append-or-clear and parallel task records merge-or-clear; YAML cannot redefine private channels. The state modifier uses fuel- and output-bounded MiniJinja with the four current SDK filters, typed state defaults/output and declared cleaning. Each LLM node maps current UI fixed/variable/template inputs into ADK content, builds one invocation-owned native agent with only exact node-selected configured/MCP tools, uses native structured-schema retry and projects only declared typed state. This deliberately replaces the SDK custom runnable/coercion/model-wrapper machinery rather than reproducing it. Selected sensitive/blocked tools remain fail-closed until the inner call ID can resume through the outer graph checkpoint. `EliteaGraphAgent` repairs root/invocation identity and emits the bounded result selected independently from `END`: terminal data, then the last non-empty assistant message, then a declaration-ordered non-internal state fallback. Direct agents and graphs use the same Runner/SessionService entrypoint, with a Checkpointer added only for graphs. ADK action Set/Transform/Switch/Loop/Merge patterns are candidates for YAML translation, but broad code/file/HTTP/database/email actions remain disabled behind Elitea authority and resource contracts. The compiler rejects every unimplemented node family and static breakpoint before graph construction. Elitea still owns production bootstrap/Redis registration, continuation-suffix recovery, remaining active-node semantics, nested tool/MCP authorization, static breakpoints and durable fan-in extension |
...
| Sensitive tools/HITL | `require_tool_confirmation`, call-ID decisions/fingerprints, dynamic graph interrupts/checkpoints | Compose | Initial direct-tool and pipeline pauses are implemented capability-disabled. Direct calls bind a fresh public `interrupt_id` to the ADK invocation, function-call ID, tool and canonical raw arguments while publishing only masked arguments; their complete in-turn transcript belongs in the fenced PostgreSQL `SessionService`. Runner persists the final call and confirmation events before yielding them, so pending direct state is restored from standard session events rather than a parallel interrupt table. Main atomically consumes the exact pending browser interrupt set; Rust strictly admits and rejoins the decision to the latest raw call. The ordinary assembler selects continuation before PAT redemption, requires a restorable session, and uses a one-shot model adapter without provider replanning. An approved metadata-proven read executes through native `RunConfig` confirmation and `ToolExecutor`; Reject and Block With Comment replace any real tool, including an effectful one, with a local no-effect adapter that emits the structured `sensitive_tool_blocked` result under the original call ID. Direct restart before or after result persistence is proven; approved effects remain closed pending durable intent/outcome receipts and reconciliation. Pipeline calls bind the public ID to the latest persisted interrupt event plus exact private checkpoint thread, graph checkpoint, pending node and definition digest. The pipeline assembler now proves authorized pause/resume/completion through the common Runner and combined claim-fenced state backend without a separate interrupt table; recovery after a continuation event but before graph advancement remains gated. Identical name and arguments can be a new call; neither arguments nor tool name alone authorize it. A separate receipt table remains unjustified until deterministic decision events/checkpoint transitions are proven insufficient |
