...
| Graph | `StateGraph`, `StateSchema`, native reducers, custom `Node`, nested `LlmAgent`, native `ToolContext`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The complete-document compiler/assembler admits strict stored `hitl`, `state_modifier`, `llm` and configured `toolkit` nodes; the separately bounded parallel core is ready for later compiler composition. YAML, state, routes and behavior-bearing digests are validated before model/tool/state construction. HITL uses ADK checkpoint/resume and remains distinct from tool confirmation. Runtime-owned channels use native reducers. LLM nodes bind one invocation-owned agent with exact node-selected toolsets and native structured retry. Toolkit nodes bind one exact read-only ADK tool, preserve parent context and project only declared typed state without a model turn. Rust deliberately omits the SDK custom runnable/coercion/model-wrapper and broad `FunctionTool` exception machinery. Sensitive tools and direct effects remain fail-closed until graph call identity and effect reconciliation can resume the outer checkpoint. `EliteaGraphAgent` repairs root identity and selects terminal data, the last assistant message or deterministic state fallback independently from `END`. Direct agents and graphs share Runner/SessionService; only graphs add Checkpointer. Unsupported nodes and static breakpoints fail before construction. Production bootstrap/Redis, continuation-suffix recovery, remaining active nodes, graph MCP authorization, static breakpoints and durable fan-in remain gates |
