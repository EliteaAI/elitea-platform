...
| --- | --- | --- | --- |
| Agent and runner | `LlmAgent`, `Runner`, `RunnerConfig`, `Event` | Adopt behind the sealed one-shot invocation state | Main claim/authorize fence, supervised lifetime, progress/terminal output and Redis retirement remain Elitea-owned. The capability-disabled native lifecycle now composes that complete owner for the ordinary-text application/ad-hoc profile. One shared strict assembler consumes the post-authorization PAT once, binds the existing Main OpenAI-compatible endpoint, and builds an invocation-local `LlmAgent`, in-memory session and exclusive `Runner`; history reconstruction, tools and capability registration remain closed |
...
| Sessions | `SessionService` and Runner session integration | Adopt interface, replace persistence where required | The capability-disabled ordinary first-turn profile uses one fresh `InMemorySessionService`, stable pseudonymous user/session identities and exactly one seeded current-user message. Nonempty history, continuation and shared memory remain rejected until an Elitea adapter proves deterministic ordering, SQL-bounded reads, metadata fidelity, concurrent deltas and compaction reconstruction |
