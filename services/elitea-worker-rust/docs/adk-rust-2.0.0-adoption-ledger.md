...
| --- | --- | --- | --- |
| Agent and runner | `LlmAgent`, `Runner`, `RunConfig`, `Event`, `AgentTool` | Adopt behind the sealed one-shot invocation state | Main claim/authorize fence, supervised lifetime, progress/terminal output and Redis retirement remain Elitea-owned. Application `agent_type=agent` and ad-hoc both build one direct invocation-local `LlmAgent`; configured native toolsets and variable-free saved direct children join the same loop, with children wrapped as `AgentTool`. Runtime/admin policy is projected into `require_tool_confirmation` on each owning `LlmAgent`. Stored pipelines remain graph-owned. Durable direct transcript/call replay, MCP authorization, active context management and capability registration remain closed |
...
| Sessions | `SessionService` and Runner session integration | Adopt the interface with an Elitea PostgreSQL adapter | `src/state/postgres_session.rs` and Main migration `0065_agent_sessions.sql` implement a bounded `adk-session.2.0.0.v1` lineage in the existing `agentstate` schema. It preserves complete ADK events, deterministic ordinals, app/user/session state tiers, exact event replay and current-claim writer fencing. Direct and graph agents both use this conversation/session contract; a graph agent additionally uses the separate `Checkpointer` contract for workflow frontier/state/interrupts. `AUTHORIZED_NOW` now mints a non-cloneable session grant and the common Runner accepts an injected claim-fenced PostgreSQL service, restoring an existing session or seeding deterministic frozen history once. Deployment bootstrap still selects the invocation-local backend until the shared pool/restricted role is wired; Main transcript ownership cutover, restart/pending-call resume and live PostgreSQL proof remain gates |
...
| Sensitive tools/HITL | `require_tool_confirmation`, call-ID decisions/fingerprints, dynamic graph interrupts/checkpoints | Compose | Initial direct-tool and pipeline pauses are implemented capability-disabled. Direct calls bind a fresh public `interrupt_id` to the ADK invocation, function-call ID, tool and canonical raw arguments while publishing only masked arguments; their complete in-turn transcript belongs in the fenced PostgreSQL `SessionService`. Pipeline calls additionally bind the public ID to thread, graph checkpoint and definition digest. Resume decisions are one-use and unknown actions fail closed. Production resume must restore the exact pending call rather than ask the model to recreate it, then atomically consume the Main-validated decision under the same execution authority. Identical name and arguments can be a new call; neither arguments nor tool name alone authorize it |
