...
| Tools/toolsets | `Tool`, `Toolset`, `BasicToolset`, typed tool macro | Adopt and wrap | Elitea owns saved configuration, selected-tool identity, grants, sensitive policy, absolute deadline, bounded concurrency and result projection. The capability-disabled GitHub reference family parses the claim-materialized SDK shape and shares one invocation-scoped pooled client between its real probe and eighteen explicitly selected native ADK reads: identity, branches, single/batch text files, bounded grep, base/active/directory repository navigation, issue query/detail/search, pull-request list/detail/file inspection, commit list/detail/compare and server-side code search. It preserves the SDK's 1-indexed slicing, 200,000-character guidance, cumulative skip, recursive Git-tree, callable issue/PR/commit/search schemas and success field meaning while moving retry ownership out of the blocking SDK helper and bounding paths, decoded files, tree completeness, issue pages, PR details/files, commit pages, patches, regex/search work and diagnostics. Rust intentionally returns typed PR comment/commit arrays, canonical UTC commit filters, complete bounded commit projections, actual code-search completeness and one direct provider page instead of Python-repr strings, unchecked time ranges, silently incomplete collections or lazy-page replay. Empty selection still rejects the unported catalog. Main retains the public check API/auth/revision/audit contract; live dispatch, the remaining 26 tools, App installation auth, HITL and indexing remain gates |
