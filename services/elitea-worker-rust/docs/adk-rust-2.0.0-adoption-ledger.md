...
| Graph | `StateGraph`, custom `Node`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The bounded `parallel` core and an initial complete-document compiler for strict stored `type: hitl` nodes are implemented. The authorized application selector now admits only frozen `agent_type=pipeline` definitions and compiles their complete YAML before any model, tool or state backend is constructed. Pipeline HITL uses ADK dynamic checkpoint/resume, exact definition digests, one-use private decisions and fail-closed routes; it is not sensitive-tool confirmation and carries no tool-call ID. `EliteaGraphAgent` repairs root/invocation identity before browser projection. The compiler rejects configured tools, every unimplemented node family and static breakpoint before graph construction. Elitea still owns combined claim-fenced PostgreSQL session/checkpoint assembly, general node/edge/reducer policy, static breakpoint semantics and durable fan-in extension |
