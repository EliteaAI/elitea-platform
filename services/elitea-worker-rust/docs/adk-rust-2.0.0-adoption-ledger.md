...
| Fixed subagents and dynamic saved-participant calls | `SequentialAgent`, `LoopAgent`, `ParallelAgent`; `LlmAgent` `ToolExecutionStrategy` | Adopt where semantics match | Fixed ADK agents remain candidates for declared static orchestration. Current ad-hoc fan-out is instead model-selected tool dispatch: a complete Application-only root tool set opts into native `LlmAgent` parallel execution with an eight-call run ceiling, while mixed toolsets stay sequential. The projector joins the frozen display name to provider call ID and sibling ordinal for stable root accordions. Stock `AgentTool` consumes child events internally, so descendant breadcrumb extension and nested HITL/auth aggregation remain gates. Both are distinct from the capability-closed future explicit pipeline `parallel` node |
