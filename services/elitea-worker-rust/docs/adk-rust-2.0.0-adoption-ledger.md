...
| Event stream | `EventStream`, `Event`, `Content`, `Part` | Adopt as internal semantic input, not as the browser/output contract | The closed ordinary-text projector now maps bounded root-model text/thinking events to current `NodeEventV1` shapes and drains through EOS. Elitea's claim-bound output cursor now owns exact fence/sequence binding and advances only after a bound ACK. Session reconnect, sanitization, durable send composition and tool/HITL/MCP/application completion projection remain Elitea-owned gates |
