...
| Graph | `StateGraph`, `StateSchema`, native reducers, custom `Node`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The bounded `parallel` core and a complete-document compiler/assembler for strict stored `type: hitl` plus `type: state_modifier` nodes are implemented. The authorized application selector compiles complete frozen `agent_type=pipeline` YAML before any model, tool or state backend is constructed. Pipeline HITL uses ADK dynamic checkpoint/resume, exact definition digests, one-use private decisions and fail-closed routes; it is not sensitive-tool confirmation and carries no tool-call ID. Runtime-owned state uses ADK reducers: `messages` appends, HITL decisions append-or-clear and parallel task records merge-or-clear; YAML cannot redefine private channels. The state modifier uses fuel- and output-bounded MiniJinja with the four current SDK filters, typed state defaults/output and declared cleaning. `EliteaGraphAgent` repairs root/invocation identity and emits the bounded result selected independently from `END`: terminal data, then the last non-empty assistant message, then a declaration-ordered non-internal state fallback. Direct agents and graphs use the same Runner/SessionService entrypoint, with a Checkpointer added only for graphs. ADK action Set/Transform/Switch/Loop/Merge patterns are candidates for YAML translation, but broad code/file/HTTP/database/email actions remain disabled behind Elitea authority and resource contracts. The next LLM-node slice must instantiate a nested ADK `LlmAgent` with only the UI-connected names resolved from claim-authorized toolsets; it must not inherit ad-hoc scope. The compiler rejects every unimplemented node family and static breakpoint before graph construction. Elitea still owns production bootstrap/Redis registration, continuation-suffix recovery, remaining active-node semantics, static breakpoints and durable fan-in extension |
