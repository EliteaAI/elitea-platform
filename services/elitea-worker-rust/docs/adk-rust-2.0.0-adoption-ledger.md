...
| Fixed subagents and dynamic saved-participant calls | `SequentialAgent`, `LoopAgent`, `ParallelAgent`; `LlmAgent` `ToolExecutionStrategy` | Adopt where semantics match | Fixed ADK agents remain candidates for declared static orchestration. Current ad-hoc fan-out is model-selected tool dispatch: a complete Application-only tool set at the root or a nested direct child opts into native `LlmAgent` parallel execution with an eight-call run ceiling, while mixed toolsets stay sequential. A bounded typed child-event adapter lets the projector build stable recursive accordions from frozen display name, provider call ID and sibling ordinal even when container and leaf calls overlap. Durable child sessions, exact-leaf HITL/OAuth aggregation and partial sibling replay remain gates. This is distinct from the capability-closed future explicit pipeline `parallel` node |
