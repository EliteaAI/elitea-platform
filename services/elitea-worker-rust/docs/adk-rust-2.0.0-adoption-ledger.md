...
| Graph | `StateGraph`, `StateSchema`, native reducers, custom `Node`, nested `LlmAgent`, native `ToolContext`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The complete-document compiler/assembler admits strict stored `hitl`, `state_modifier`, `llm`, configured `toolkit` and direct remote `mcp` nodes; the separately bounded parallel core is ready for later compiler composition. YAML, state, routes and behavior-bearing digests are validated before model/tool/state construction. Configured HITL and sensitive direct-tool confirmation use distinct schemas over the same ADK checkpoint mechanism. Runtime-owned channels use native reducers. LLM nodes bind one invocation-owned agent with exact node-selected toolsets and native structured retry. Toolkit and auth-free remote MCP nodes bind one exact read-only ADK tool, preserve parent context and project typed state without a model turn. A sensitive read pauses before dispatch; approval returns normal output, while denial records a same-call structured block, preserves the SDK user explanation and routes to `END` without corrupting typed state. Rust omits the SDK custom runnable/coercion/model-wrapper and broad exception machinery. Effects, nested LLM-node confirmation and MCP OAuth/on-demand authorization remain fail-closed. `EliteaGraphAgent` repairs root identity and selects terminal data, last assistant message or deterministic state independently from `END`. Direct agents and graphs share Runner/SessionService; only graphs add Checkpointer. Production bootstrap/Redis, continuation-suffix recovery, remaining nodes, static breakpoints and durable fan-in remain gates |
