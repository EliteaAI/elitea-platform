...
| Tools/toolsets | `Tool`, `Toolset`, `BasicToolset`, typed tool macro | Adopt and wrap | Elitea owns saved configuration, selected-tool identity, grants, sensitive policy, absolute deadline, bounded concurrency and result projection. The capability-disabled GitHub reference family parses the claim-materialized SDK shape and shares one invocation-scoped pooled client between its real probe and eleven explicitly selected native ADK reads: identity, branches, single/batch text files, bounded grep, base/active/directory repository navigation and issue query/detail/search. It preserves the SDK's 1-indexed slicing, 200,000-character guidance, cumulative skip, recursive Git-tree and callable issue-schema contracts while moving retry ownership out of the blocking SDK helper and bounding paths, decoded files, tree completeness, issue pages, regex work and diagnostics. Empty selection still rejects the unported catalog. Main retains the public check API/auth/revision/audit contract; live dispatch, the remaining 33 tools, App installation auth, HITL and indexing remain gates |
