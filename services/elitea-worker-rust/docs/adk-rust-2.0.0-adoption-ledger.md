...
| Models | Provider-neutral `Llm`; optional OpenAI/Anthropic and other feature gates | Wrap | Construct the provider only after the strict authorized profile redeems credentials through `transport/runtime_context.rs`. The origin-bound mTLS HTTP/2 redemption client and zeroized claim-scoped context are implemented. Initial Elitea gateway support still requires an Elitea-owned bounded/redacted `Llm` transport: ADK 2.0.0's bundled OpenAI clients construct their own general `reqwest` client and cannot receive the worker private-CA/mTLS policy. Native Anthropic requires a header/routing compatibility adapter rather than silent coercion |
