...
Graph, Event and Sensitive/HITL rows means LLM-owned auth-control replay,
parallel exact-card continuation, and concrete unported configured families.
It does not include the direct-node card/Skip path or remote-MCP exact-server
Authorize rebuild, which are implemented as described in the dedicated row.
