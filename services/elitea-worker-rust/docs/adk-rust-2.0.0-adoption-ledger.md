...
| Sessions | `SessionService` and Runner session integration | Adopt the interface with an Elitea PostgreSQL adapter | `src/state/postgres_session.rs` and Main migration `0065_agent_sessions.sql` implement a bounded `adk-session.2.0.0.v1` lineage in the existing `agentstate` schema. It preserves complete ADK events, deterministic ordinals, app/user/session state tiers, exact event replay and current-claim writer fencing. Direct and graph agents both use this conversation/session contract; a graph agent additionally uses the separate `Checkpointer` contract for workflow frontier/state/interrupts. Current direct assembly still uses the in-memory service until immutable session identity/digest derivation and claim-bound injection are composed; frozen-history seed-once, restart/resume and restricted-role PostgreSQL proof remain gates |
