...
| --- | --- | --- | --- |
| Agent and runner | `LlmAgent`, `Runner`, `RunConfig`, `Event`, `AgentTool` | Adopt behind the sealed one-shot invocation state | Main claim/authorize fence, supervised lifetime, progress/terminal output and Redis retirement remain Elitea-owned. Application `agent_type=agent` and ad-hoc both build one direct invocation-local `LlmAgent`; configured native toolsets and variable-free saved direct children join the same loop, with children wrapped as `AgentTool`. Runtime/admin policy is projected into `require_tool_confirmation` on each owning `LlmAgent`. Stored pipelines remain graph-owned. A capability-disabled read-only HITL proof restores a persisted call in a second Runner, re-emits it without provider replanning and applies exact `RunConfig` confirmation before native tool execution. Production continuation admission, partial-replay restart recovery, effect ownership, MCP authorization, active context management and capability registration remain closed |
