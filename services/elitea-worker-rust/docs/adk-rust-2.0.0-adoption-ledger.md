...
| Event stream | `EventStream`, `Event`, `Content`, `Part` | Adopt as internal semantic input, not as the browser/output contract | The projector maps bounded root-model text/thinking/tool events, exact direct confirmation pauses, configured graph HITL and sensitive Toolkit-node interrupts to current `NodeEventV1` shapes and drains through EOS. Sensitive inputs are retained privately for identity but masked publicly; graph Toolkit cards bind the checkpoint/pending node/definition/visit-specific call and canonical argument digest. Elitea's capability-disabled lifecycle retains the command-bound cursor, encrypted spool, live/restored session, one pending frame and attempt budget while ACKing each batch before polling ADK again. A pause is durably ACKed before `PAUSED_HITL`. MCP/skill/static-breakpoint projection and production resume remain gates |
...
| Observability | `tracing` instrumentation below ADK plus Elitea lifecycle spans | Wrap at orchestration/tool boundaries | The binary installs a crate-only level-filtered subscriber. Authenticated prepare/lifecycle, provider-neutral assembly, configured tool calls, nested `AgentTool` calls and pipeline LLM/Toolkit nodes carry safe correlation, phase, outcome, duration and stable code fields. Node spans record only node identity, selected-tool counts, structured mode and stage; prompts, history, arguments, results, URLs, credentials and provider bodies are excluded. OTLP/metrics export and deployed retention/cardinality policy remain the next local-platform gate |
...
| Graph | `StateGraph`, `StateSchema`, native reducers, custom `Node`, nested `LlmAgent`, native `ToolContext`, `interrupt_with_data`, deferred nodes, bounded compiled concurrency | Adopt compiler substrate | The complete-document compiler/assembler admits strict stored `hitl`, `state_modifier`, `llm` and configured `toolkit` nodes; the separately bounded parallel core is ready for later compiler composition. YAML, state, routes and behavior-bearing digests are validated before model/tool/state construction. Configured HITL and sensitive direct Toolkit confirmation use distinct schemas over the same ADK checkpoint mechanism. Runtime-owned channels use native reducers. LLM nodes bind one invocation-owned agent with exact node-selected toolsets and native structured retry. Toolkit nodes bind one exact read-only ADK tool, preserve parent context and project typed state without a model turn. A sensitive read pauses before dispatch; approval returns normal output, while denial records a same-call structured block, preserves the SDK user explanation and routes to `END` without corrupting typed state. Rust omits the SDK custom runnable/coercion/model-wrapper and broad exception machinery. Effects, nested LLM-node confirmation and MCP authorization remain fail-closed. `EliteaGraphAgent` repairs root identity and selects terminal data, last assistant message or deterministic state independently from `END`. Direct agents and graphs share Runner/SessionService; only graphs add Checkpointer. Production bootstrap/Redis, continuation-suffix recovery, remaining nodes, static breakpoints and durable fan-in remain gates |
