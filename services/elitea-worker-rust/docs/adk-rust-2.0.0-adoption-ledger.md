...
| Sensitive tools/HITL | `require_tool_confirmation`, call-ID decisions/fingerprints, dynamic graph interrupts/checkpoints | Compose | Initial direct-tool and pipeline pauses are implemented capability-disabled. Direct calls bind a fresh public `interrupt_id` to the ADK invocation, function-call ID, tool and canonical raw arguments while publishing only masked arguments; their complete in-turn transcript belongs in the fenced PostgreSQL `SessionService`. Runner persists the final call and confirmation events before yielding them, so pending direct state is restored from standard session events rather than a parallel interrupt table. Main already atomically consumes the exact pending browser interrupt set and submits a continuation; Rust now strictly admits that decision and independently resolves it against the latest raw persisted call. The resolved value is deliberately not execution authority. Pipeline calls additionally bind the public ID to thread, graph checkpoint and definition digest. Production direct resume must execute the restored call without asking the model to recreate it and must own effect receipts/reconciliation. Identical name and arguments can be a new call; neither arguments nor tool name alone authorize it. A separate receipt table remains unjustified until deterministic decision events/checkpoint transitions are proven insufficient |
