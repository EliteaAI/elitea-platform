...
| --- | --- | --- | --- |
| Agent and runner | `LlmAgent`, `Runner`, `RunnerConfig`, `Event` | Adopt behind the sealed one-shot invocation state | Main claim/authorize fence, supervised lifetime, progress/terminal output and Redis retirement remain Elitea-owned. The capability-disabled native lifecycle now composes that complete owner for the ordinary-text application/ad-hoc profile. A strict no-tool admission typestate consumes its post-authorization claim-scoped PAT redemption exactly once, and the first fresh-turn foundation builds one invocation-local `LlmAgent`, in-memory session and exclusive `Runner`; production model binding, history reconstruction and tool assembly remain closed |
