...
if AUTO_TARGET_DOCS:
    logger.info("[FEATURE FLAG] DEEPWIKI_AUTO_TARGET_DOCS=1: Docs auto-retrieved during generation")
...
_TRIVIAL_SIG_TOKENS = frozenset({
    'self', 'cls', 'this', 'public', 'private', 'protected', 'static',
    'final', 'abstract', 'virtual', 'override', 'inline', 'extern',
    'async', 'def', 'function', 'fn', 'func', 'void',
})
...
def _signature_brief(node_data: Dict, max_len: int = 120) -> str:
    ...
    if symbol and hasattr(symbol, 'parameter_types'):
        param_types = [p for p in (symbol.parameter_types or [])
                       if p.lower() not in _TRIVIAL_SIG_TOKENS]
    if not param_types:
        param_types = [p for p in (node_data.get('parameters') or [])
                       if p.lower() not in _TRIVIAL_SIG_TOKENS]
...
class PageDefinition(BaseModel):
    ...
    section_name: str = Field(..., description="Name of the section this page belongs to (must match a defined section)")
    page_name: str = Field(
        ..., 
        description=(
    ...
    target_folders: List[str] = Field(default_factory=list, description="List of folders this page covers (e.g., ['src/auth', 'lib/security'])")
    key_files: List[str] = Field(default_factory=list, description="Key files to reference (e.g., ['README.md', 'config.py'])")
    retrieval_query: str = Field(default="", description="Fallback search query for documentation files. target_symbols is preferred for code retrieval.")
...
STRUCTURE_MAX_DEPTH = _get_env_int("DEEPWIKI_STRUCTURE_MAX_DEPTH", 5)
MAX_TREE_DEPTH = STRUCTURE_MAX_DEPTH
# Absolute ceiling — never exceed this even with auto-detection
_DEPTH_CEILING = _get_env_int("DEEPWIKI_STRUCTURE_MAX_DEPTH_CEILING", 8)
...
MAX_TREE_ENTRIES = _get_env_int("DEEPWIKI_STRUCTURE_MAX_ENTRIES", 1000)


def detect_effective_depth(repository_files: List[str],
                          static_default: int = STRUCTURE_MAX_DEPTH,
                          ceiling: int = _DEPTH_CEILING) -> int:
    ...
    if effective != static_default:
        logger.info(
            "[DEPTH_AUTO] Effective depth adjusted: %d → %d "
            "(p90_code_depth=%d, files_sampled=%d, ceiling=%d)",
            static_default, effective, p90_depth, len(code_depths), ceiling,
        )
...
def _is_code_file(name: str) -> bool:
    ...
    return ext in {
        '.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.go', '.rs', '.rb',
        '.cpp', '.cc', '.cxx', '.c++', '.c', '.h', '.hpp', '.hh', '.hxx',  # C/C++ complete set
        '.cs', '.swift', '.kt', '.kts', '.scala', '.php',
        '.vue', '.svelte', '.lua', '.r', '.jl', '.ex', '.exs', '.erl', '.hs',
        '.clj', '.cljs', '.ml', '.fs', '.fsx', '.dart', '.groovy', '.gradle',
    }
...
def _should_ignore_dir(name: str) -> bool:
    ...
    return (
        name in IGNORED_DIRS or
        name.startswith('.') and name not in DOT_DIR_WHITELIST or
        name.startswith('_') and name != '__init__.py' or
    ...
    try:
        for item in sorted(os.listdir(repo_root)):
            if os.path.isfile(os.path.join(repo_root, item)):
                ...
                if (item_lower.startswith('readme') or
                    item_lower.startswith('contributing') or
                    item_lower.startswith('changelog') or
                    item_lower in ('license', 'makefile', 'justfile', 'dockerfile',
    ...
    if large_dirs:
        ...
        for parent in large_sorted[:8]:
            ...
            content_descendants = [
                ...
                if d["path"].startswith(parent_path + "/") and d["files"] > 0
            ]
            ...
            parent_counts = Counter(
                d["path"].rsplit("/", 1)[0] for d in content_descendants
            )
            for dir_parent, count in parent_counts.most_common(3):
                if count >= 15:
                    ...
                    lines.append(f"  → GROUP multiple subdirs into themed pages, not one per subdirectory")
...
class StructureCollector:
    ...
    def __init__(self, page_budget: int = 20, repo_root: Optional[str] = None,
                 code_graph: Optional[Any] = None, graph_text_index: Optional[Any] = None,
        ...
        self.page_budget_overflow = _get_env_int("DEEPWIKI_PAGE_BUDGET_OVERFLOW", overflow_default)
        ...
        self._max_symbols_per_page = OptimizedWikiGenerationAgent._compute_max_symbols_per_page(_gn, repository_file_count)
        
        # Deep exploration cache
        self._tree_analysis: Optional[Dict] = None
    ...
    def register_discovered_dirs(self, dirs: List[str]):
        ...
        (e.g., 'elitea_sdk/tools/ado'). Leaf-only entries from ls may
    ...
    def _format_coverage_feedback(self) -> str:
        ...
        feedback_parts.append(
            f"→ query_graph each SPECIFIC path above, then add pages."
        )
    ...
    def _handle_define_section(
        ...
        warnings = []
        generic_patterns = [
            'core', 'service', 'services', 'clients', 'utils', 'helpers',
            'components', 'modules', 'framework', 'external'
        ]
    ...
    def _handle_define_page(
        ...
        logger.info(f"[STRUCTURE_PLANNER][TOOL] define_page: {page_name} in {section_name} ({page_num}/{self.page_budget})")
        ...
        if not target_symbols:
            warnings.append("⚠️ No target_symbols provided. Use class/function names from query_graph for precise code retrieval.")
        elif len(target_symbols) < 5:
            warnings.append(f"⚠️ Only {len(target_symbols)} target_symbols — pages should have 10-15+ symbols. Merge related classes, functions, constants, and helpers from the same capability into this page.")
        elif len(target_symbols) > self._max_symbols_per_page + 15:
            warnings.append(f"⚠️ Very high target_symbols count ({len(target_symbols)}). Consider splitting into 2-3 capability-focused pages with 15-{self._max_symbols_per_page} symbols each.")
        ...
        if AUTO_TARGET_DOCS:
            ...
            if target_docs:
                logger.info(f"[STRUCTURE_PLANNER] AUTO_TARGET_DOCS mode: {len(target_docs)} explicit target_docs provided")
            else:
                logger.debug("[STRUCTURE_PLANNER] AUTO_TARGET_DOCS mode: target_docs empty, will auto-retrieve during generation")
            ...
            if docs_available:
                ...
                warnings.append("💡 No target_docs specified. If query_graph showed DOCUMENTATION FILES for this module, add relevant ones.")
        else:
            ...
            for doc_path in target_docs:
                ...
                doc_keywords = set(doc_name.replace('.md', '').replace('.rst', '').replace('-', ' ').replace('_', ' ').split())
        ...
        if target_symbols and self.code_graph:
            ...
            if not hasattr(self, '_case_insensitive_symbols'):
                ...
                for node_id, node_data in self.code_graph.nodes(data=True):
                    sym_type = (node_data.get('symbol_type') or
                                node_data.get('type') or '').lower()
            ...
            for sym in target_symbols:
                ...
                if name_index and sym in name_index:
                    ...
                    arch_nodes = [
                        nid for nid in name_index[sym]
                        if (self.code_graph.nodes.get(nid, {}).get('symbol_type') or '').lower()
                        in CODE_SYMBOL_TYPES
                    ]
                    if arch_nodes:
                        found = True
                    else:
                ...
                # Check case-insensitive match (already filtered to arch types)
                    ...
                    for node_id, node_data in list(self.code_graph.nodes(data=True))[:2000]:
                        ...
                        if (sym_name.lower() == sym.lower() or 
                            node_suffix.lower() == sym.lower() or
                            sym_name.lower().endswith(f".{sym.lower()}")):
                            found = True
            ...
            if unvalidated_symbols:
                ...
                logger.warning(f"[STRUCTURE_PLANNER] target_symbols not in graph: {unvalidated_symbols}, suggestions: {symbol_suggestions}")
        ...
        if validated_symbols:
            ...
            if overlap_symbols:
                ...
                logger.info(
                    f"[STRUCTURE_PLANNER] Overlapping symbols on '{page_name}': "
                    f"{overlap_symbols}"
                )
        ...
        page_data = {
            ...
            "rationale": rationale,
            "target_symbols": validated_symbols if validated_symbols else [],  # Only validated symbols — never fall back to unvalidated LLM input
    ...
    def _handle_batch_define_pages(self, pages: List[Any]) -> str:
        ...
        # Log summary
        logger.info(f"[STRUCTURE_PLANNER][TOOL] batch_define_pages: Created {success_count} pages, {error_count} errors")
        ...
        if coverage_pct < 90 and (remaining > 0 or self.page_budget_overflow > 0):
            needs_continue = True
            budget_info = f"{remaining} pages remaining" if remaining > 0 else f"overflow budget: +{self.page_budget_overflow}"
            summary_parts.append("=" * 60)
            summary_parts.append(f"🚨🚨🚨 STOP — DO NOT SAY \"DONE\" 🚨🚨🚨")
            summary_parts.append(f"")
            summary_parts.append(f"COVERAGE: {coverage_pct:.0f}% ({covered_count}/{discovered_count} dirs) — TARGET: 90%")
            ...
            if uncovered_top_dirs:
                ...
                summary_parts.append(f"UNCOVERED AREAS — query_graph these SPECIFIC paths, then batch_define_pages:")
                ...
                summary_parts.append(f"")
            summary_parts.append(f"ACTION: Call query_graph() for uncovered areas, then batch_define_pages with 10-15 more pages.")
            summary_parts.append("=" * 60)
            summary_parts.append(f"")
        elif remaining > 10:
            needs_continue = True
            ...
            summary_parts.append(f"→ Call batch_define_pages again with 10-15 more pages")
            summary_parts.append(f"")
    ...
    def _handle_think(self, analysis: str) -> str:
        ...
        if self.discovered_dirs and not self.pages:
            ...
            if uncovered:
                ...
                if depth_2_uncovered:
                    warnings.append(
                        f"📋 UNCOVERED DIRECTORIES (depth 2): {', '.join(sorted(depth_2_uncovered)[:8])}"
                    )
                if depth_3_uncovered:
                    warnings.append(
                        f"📋 UNCOVERED DIRECTORIES (depth 3): {', '.join(sorted(depth_3_uncovered)[:8])}"
                    )
        ...
        if self._tree_analysis:
            ...
            if large_dirs and analyzed_count == 0 and not self.sections:
                ...
                unanalyzed = [d['name'] for d in large_dirs 
                             if d.get('path') not in self._graph_module_cache]
                if unanalyzed:
                    warnings.append(
    ...
    def _handle_query_graph(self, path_prefix: str, text_filter: str = None) -> str:
        ...
        logger.info(f"[STRUCTURE_PLANNER][TOOL] query_graph: Trying prefixes {prefixes_to_try} for {path_prefix}"
        ...
        creates_map = {}  # node_id -> list of created types (from CREATES relationship)
        ...
        def matches_any_prefix(rel_path_norm: str, prefixes: list) -> bool:
            ...
            for prefix in prefixes:
                if (rel_path_norm.startswith(prefix + '/') or
                    rel_path_norm == prefix or
        ...
        try:
            ...
            if fts is not None and fts.is_open:
                ...
                tf_tokens = (
                    [t for t in tf_lower.split() if len(t) >= 2]
                    if tf_lower else None
                )
                for node_id, node_data in self.code_graph.nodes(data=True):
                    rel_path = node_data.get('rel_path') or node_data.get('file_path', '')
                    if not rel_path:
                    ...
                    if tf_tokens:
                        sym_name = (node_data.get('symbol_name', '')
                                    or node_data.get('name', '') or '').lower()
            ...
            for node_id, node_data in candidate_nodes:
                ...
                symbol_type = (node_data.get('symbol_type') or 
                              node_data.get('type') or 
                              node_data.get('node_type', 'unknown')).lower()
                ...
                connections = (self.code_graph.in_degree(node_id) + 
                             self.code_graph.out_degree(node_id))
                                        ...
                                        for impl_node in self.code_graph.predecessors(succ):
                                            ...
                                            if impl_edge:
                                                for impl_e in impl_edge.values():
                                                    if str(impl_e.get('relationship_type', '')).lower() == 'defines_body':
                                                        ...
                                                        if impl_file and impl_file != rel_path:
                                                            ...
                                                            if impl_rel not in implemented_in and len(implemented_in) < 3:
        ...
        class_count = len([s for t, syms in symbols_by_type.items() 
                          if t in ('class', 'interface', 'protocol', 'struct', 'trait') 
                          for s in syms])
        function_count = len([s for t, syms in symbols_by_type.items() 
                             if t in ('function', 'method', 'def') 
                             for s in syms])
        ...
        logger.info(
            f"[STRUCTURE_PLANNER][TOOL] query_graph: {path_prefix} -> {', '.join(parts)}"
        )
    ...
    def _generate_grouping_suggestions(
        ...
        for file_name, file_symbols in symbols_by_file.items():
            ungrouped_enums = [
                ...
                if s['type'] in ('enum', 'constant', 'type_alias') 
                and s['name'] not in used_symbols
            ]
    ...
    def _format_graph_analysis(self, path_prefix: str, analysis: Dict) -> str:
        ...
        if all_classes:
            ...
            classes_with_context = [c for c in all_classes if 
                                   c.get('inherits_from') or c.get('implemented_by') or c.get('created_by') or
                                   c.get('specializes') or c.get('specialized_by') or
                                   c.get('declared_in') or c.get('implemented_in') or c.get('has_split_impl') or
                                   c.get('uses_types')]
        ...
        if by_type:
            type_summary = ", ".join(f"{stype}: {count}" 
    ...
    def _handle_search_symbols(
        ...
        if fts is None or not fts.is_open:
            ...
            logger.info(
                "[STRUCTURE_PLANNER][TOOL] search_symbols: FTS5 unavailable, "
                "falling back to search_graph"
            )
        ...
        logger.info(
            f"[STRUCTURE_PLANNER][TOOL] search_symbols: "
            f"query={query!r}, type={symbol_type}, path={path_prefix}"
        )
        ...
        logger.info(
            f"[STRUCTURE_PLANNER][TOOL] search_symbols: "
            f"{len(docs)} results for {query!r}"
        )

        return self._format_search_symbols_results(
            query, docs, symbol_type, path_prefix,
        )
    ...
    def _format_search_symbols_results(
        ...
        type_order = ['class', 'interface', 'struct', 'enum', 'trait',
                      'function', 'constant', 'type_alias']
        _PLURAL = {
            'class': 'CLASSES', 'interface': 'INTERFACES', 'struct': 'STRUCTS',
            'enum': 'ENUMS', 'trait': 'TRAITS', 'function': 'FUNCTIONS',
    ...
    def _handle_search_graph(self, pattern: str, symbol_type: Optional[str] = None, include_docstrings: bool = False) -> str:
        ...
        try:
            ...
            if self.query_service and (self.graph_text_index and self.graph_text_index.is_open):
                ...
                for r in results:
                    ...
                    matches.append(m)
                logger.info(
                    f"[STRUCTURE_PLANNER][TOOL] search_graph: FTS5 returned "
                ...
                for node_id, node_data in self.code_graph.nodes(data=True):
                    ...
                    stype = (node_data.get('symbol_type') or 
                            node_data.get('type') or 
                            node_data.get('node_type', 'unknown')).lower()
    ...
    def _format_search_results(self, pattern: str, matches: List[Dict], symbol_type: Optional[str], include_docstrings: bool = False) -> str:
        ...
        for stype, syms in by_type.items():
            ...
            for sym in syms[:5]:
                desc_str = f" — {sym['desc']}" if sym.get('desc') else ""
                lines.append(f"  • {sym['name']} in {sym['path'].split('/')[-2] if '/' in sym['path'] else sym['path']}{desc_str}")
            if len(syms) > 5:
                lines.append(f"  ... and {len(syms) - 5} more")
            lines.append("")
    ...
    def get_tools(self) -> List:
        ...
        def search_graph(pattern: str, symbol_type: str = "", include_docstrings: bool = False) -> str:
            ...
            """
            return self._handle_search_graph(
                pattern, 
        ...
        def define_page(
            ...
            return self._handle_define_page(
                section_name, page_name, page_order, description,
                content_focus, rationale, target_symbols, target_docs, target_folders, key_files, retrieval_query
            )
        ...
        return [explore_tree, analyze_module, think, query_graph, search_graph, search_symbols, set_wiki_metadata, define_section, define_page, batch_define_pages]
    ...
    def assemble(self) -> Dict[str, Any]:
        ...
        if not self.metadata:
            self.metadata = {
                ...
                "overview": "Repository documentation"
            }
        ...
        sorted_sections = sorted(
            ...
            key=lambda s: s.get("section_order", 99)
        )
        ...
        for section in sorted_sections:
            section["pages"] = sorted(
                section["pages"],
                key=lambda p: p.get("page_order", 99)
            )
