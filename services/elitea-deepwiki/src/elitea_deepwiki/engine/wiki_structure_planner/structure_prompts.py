...
def _format_doc_clusters_section(doc_clusters: List[Dict[str, Any]]) -> str:
    ...
    for cluster in doc_clusters:
        if cluster.get("type") == "collection":
            ...
            lines.append(f"### `{cluster['path']}/` - Collection of {cluster['child_count']} items ({cluster['total_files']} files)")
            lines.append("")
            ...
            lines.append("")
            lines.append(f"**ACTION**: Create a section or page for `{cluster['path']}` with target_paths pointing to the individual directories.")
            lines.append("")
        else:
            # Single doc directory
            lines.append(f"- `{cluster['path']}/` ({cluster['file_count']} files: {', '.join(cluster.get('doc_types', []))})")
...
def get_structure_task_prompt(
    page_budget: int = 20,
    repo_name: str = "",
    doc_clusters: list = None
) -> str:
