...
def _load_pickle_graph(cache_file: Path) -> nx.DiGraph:
    """Load a graph from a .code_graph.gz file.

    Tries gzip-compressed pickle first, then falls back to raw pickle.
    The fallback is needed because the platform artifact HTTP API may
    auto-decompress gzip content (via Content-Encoding: gzip), stripping
    the gzip wrapper and leaving raw pickle bytes on disk.
    """
    try:
        ...
        logger.debug(
            "File %s is not gzip-compressed — loading as raw pickle (likely "
            "auto-decompressed during HTTP download)",
            cache_file.name,
        )
...
def _is_fts5_enabled() -> bool:
    ...
    Controlled by ``DEEPWIKI_ENABLE_FTS5`` environment variable.
    ...
    """
    return os.getenv("DEEPWIKI_ENABLE_FTS5", "1") != "0"
...
class GraphManager:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        self._fts_index: Optional[GraphTextIndex] = None

    @property
    def fts_index(self) -> Optional[GraphTextIndex]:
        """Lazily-created FTS5 index instance sharing the same cache dir.

        Returns ``None`` when ``DEEPWIKI_ENABLE_FTS5`` is set to ``0``.
        """
    ...
    def save_graph(self, graph: nx.DiGraph, repo_path: str, graph_type: str = "combined", 
                   commit_hash: Optional[str] = None, repo_identifier: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            try:
                self.build_fts_index(
                    graph, repo_path, graph_type, commit_hash, repo_identifier
                )
    ...
    def load_graph(self, repo_path: str, graph_type: str = "combined", 
                   commit_hash: Optional[str] = None) -> Optional[nx.DiGraph]:
        """
        ...
        try:
            ...
            # Load code_graph (handles both gzip and raw pickle)
            graph = _load_pickle_graph(cache_file)
    ...
    def _generate_cache_key(self, repo_path: str, graph_type: str, commit_hash: Optional[str] = None) -> str:
    ...
    def graph_exists(self, repo_path: str, graph_type: str = "combined", 
    ...
    def load_graph_by_repo_name(self, repo_identifier: str, graph_type: str = "combined") -> Optional[nx.DiGraph]:
        """
        ...
        try:
            logger.info(
                f"Loading {graph_type} graph from cache key={cache_key} (file={cache_file.name})"
            )
            graph = _load_pickle_graph(cache_file)
            logger.info(f"Loaded {graph_type} graph by repo name: {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges")
    ...
    def build_fts_index(
        ...
        the ``DEEPWIKI_ENABLE_FTS5`` is set to ``0``.
        """
        if not _is_fts5_enabled():
            logger.debug("FTS5 index build skipped (DEEPWIKI_ENABLE_FTS5=0)")
            return None
        cache_key = self._generate_cache_key(repo_path, graph_type, commit_hash)
        idx = self.fts_index
        ...
        logger.info(
            f"Built FTS5 index for {repo_identifier or repo_path}: "
            f"{count} nodes indexed (cache_key={cache_key})"
        )
        return idx
    ...
    def load_fts_index_by_repo_name(
        self, repo_identifier: str
    ) -> Optional[GraphTextIndex]:
