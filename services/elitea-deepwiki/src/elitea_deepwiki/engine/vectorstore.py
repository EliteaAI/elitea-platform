...
Default remains HuggingFace if no embeddings instance is provided, but callers can
inject a pre-configured Embeddings (e.g., OpenAIEmbeddings via a litellm proxy).
"""
...
# Global cache for HuggingFace embeddings instances (legacy fallback)
_EMBEDDINGS_CACHE = {}
...
def get_cached_embeddings(model_name: str, cache_folder: str):
    """Get or create a cached HuggingFaceEmbeddings instance (legacy fallback)."""
...
# Set DEEPWIKI_DISABLE_LAZY_DOCSTORE=1 to use legacy pickle-based docstore
LAZY_DOCSTORE_ENABLED = os.getenv("DEEPWIKI_DISABLE_LAZY_DOCSTORE") != "1"
...
class VectorStoreManager:
    ...
    If an `embeddings` instance is provided, it will be used directly.
    Otherwise, falls back to a cached HuggingFaceEmbeddings instance.
    """
    ...
    def __init__(self, cache_dir: Optional[str] = None,
        ...
        Args:
            ...
            model_name: Model name for embeddings (used only if `embeddings` is None)
            model_cache_dir: Directory for caching models (HuggingFace fallback)
            embeddings: Pre-configured embeddings instance (e.g., OpenAIEmbeddings)
        """
        ...
        # Set up model cache directory (for HF fallback)
        ...
        if embeddings is not None:
            ...
            if HUGGINGFACE_AVAILABLE:
                ...
                logger.info(
                    f"Using HuggingFaceEmbeddings fallback (model={model_name}, cache_dir={self.model_cache_dir})"
                )
            else:
                raise ImportError("No embeddings provided and HuggingFace embeddings not available")
    ...
    def _maybe_copy_documents(self, documents: List[Document]) -> List[Document]:
        ...
        Set DEEPWIKI_DEEPCOPY_DOCS=1 to enable deepcopy (legacy behavior).
        """
        if os.getenv("DEEPWIKI_DEEPCOPY_DOCS") == "1":
            return copy.deepcopy(documents)
        return list(documents)
        
    def load_or_build(self, documents: List[Document], repo_path: str,
                      force_rebuild: bool = False,
                      commit_hash: Optional[str] = None) -> Tuple[FAISS, List[Document]]:
        """
    ...
    def _generate_repo_hash(self, repo_path: str, documents: List[Document], commit_hash: Optional[str]) -> str:
        """Generate hash for repository content using (path + commit hash OR mtime) for stable cache identity.
        
        When DEEPWIKI_DOC_SEPARATE_INDEX=1, includes a suffix to ensure separate cache
        ...
        if LAZY_DOCSTORE_ENABLED:
            try:
                ...
                if not lazy and docs_file.exists():
                    migrated = migrate_docstore_from_docs_pickle(
                        cache_file.parent, cache_key, docs_file
                    )
                    if migrated:
                        lazy = load_docstore_cache(cache_file.parent, cache_key)
                if lazy:
                    ...
                    logger.info(
                        f"Loaded {len(self.documents)} documents with lazy docstore from {cache_file}"
                    )
    ...
    def load_by_cache_key(self, cache_key: str) -> bool:
        ...
        if not cache_file.exists() or (not has_lazy_docstore and not has_legacy_docs):
            logger.debug(f"Cache not found for key {cache_key} (faiss={cache_file.exists()}, lazy={has_lazy_docstore}, legacy={has_legacy_docs})")
            return False
    ...
    def register_cache(self, repo_identifier: str, cache_key: str) -> None:
        ...
        try:
            from .repo_resolution import ensure_refs, split_repo_identifier, repo_branch_key
    ...
    def load_by_repo_name(self, repo_identifier: str) -> Optional[FAISS]:
        """
    ...
    def _build_and_save(self, documents: List[Document], cache_file: Path, 
        ...
        for doc in documents:
            ...
            import unicodedata
            normalized = ''.join(
                ' ' if unicodedata.category(char).startswith(('Z', 'C')) else char 
                for char in content
            ).strip()
        ...
        if total_docs <= batch_size:
            ...
            try:
                ...
                for i, (text, meta) in enumerate(list(zip(texts, metadatas))[:5]):
                    logger.debug(
            ...
            for batch_index, start in enumerate(range(0, total_docs, batch_size), start=1):
                ...
                try:
                    ...
                    for doc_idx, (text, meta) in enumerate(zip(batch_texts, batch_metas)):
                        try:
            ...
            if failed_docs_total > 0:
                ...
                logger.info(
                    f"Completed embedding of {total_docs} documents in {total_batches} batches "
                    f"(elapsed={elapsed:.2f}s)"
                )
        ...
        if LAZY_DOCSTORE_ENABLED and os.getenv("DEEPWIKI_RELOAD_LAZY_DOCSTORE_AFTER_BUILD", "1") == "1":
            try:
    ...
    def _build_and_save_from_iterable(
        ...
        logger.info(
            f"Built vector store from stream with {total_docs} documents "
    ...
    def get_mmap_bm25_retriever(
        ...
        use_mmap = os.getenv("DEEPWIKI_MMAP_BM25", "1")
        if use_mmap == "0":
            return None
    ...
    def clear_cache(self, repo_path: Optional[str] = None, commit_hash: Optional[str] = None):
