...
Feature flag
~~~~~~~~~~~~
Always available — query building doesn't depend on ``DEEPWIKI_ENABLE_FTS5``.
...
CODE_SIGNIFICANT_WORDS: frozenset = frozenset({
    ...
    'module', 'modules', 'package', 'packages', 'constant', 'constants',
    'type', 'types', 'variable', 'variables', 'parameter', 'parameters',
    'error', 'errors', 'exception', 'exceptions',
    'handler', 'handlers', 'service', 'services',
    'config', 'configuration', 'factory', 'builder', 'manager',
    'client', 'server', 'request', 'response',
    'database', 'cache', 'queue', 'stream',
    'test', 'tests', 'mock', 'fixture',
    'auth', 'authentication', 'authorization',
    'api', 'rest', 'http', 'grpc',
    'async', 'await', 'promise', 'callback',
    'abstract', 'virtual', 'override', 'static',
    'template', 'generic', 'trait', 'mixin',
    'create', 'delete', 'update', 'read',
    'init', 'initialize', 'setup', 'teardown',
    'parse', 'parser', 'serialize', 'deserialize',
    'validate', 'validation', 'schema',
    'import', 'export', 'inherit', 'implement',
...
TYPE_KEYWORDS: Dict[str, str] = {
    ...
    'module': 'module_doc', 'modules': 'module_doc',
}
...
RELATIONSHIP_KEYWORDS: Dict[str, str] = {
    # OOP — inheritance
    'inherits': 'inheritance', 'inherit': 'inheritance', 'extends': 'inheritance',
    'extend': 'inheritance', 'extending': 'inheritance',
    'subclass': 'inheritance', 'subclasses': 'inheritance',
    'inheritance': 'inheritance',
    # OOP — implementation (interface)
    'implements': 'implementation', 'implement': 'implementation',
    'implementing': 'implementation', 'implementation': 'implementation',
    # OOP — composition / aggregation
    'composition': 'composition', 'composes': 'composition', 'composed': 'composition',
    'aggregation': 'aggregation', 'aggregates': 'aggregation',
    # Calls
    'calls': 'calls', 'call': 'calls', 'calling': 'calls', 'invokes': 'calls',
    # Imports / exports
    'imports': 'imports', 'import': 'imports', 'importing': 'imports',
    'exports': 'exports', 'export': 'exports',
    # Structural
    'defines': 'defines', 'define': 'defines', 'defining': 'defines',
    'contains': 'contains', 'contain': 'contains',
    'decorates': 'decorates', 'decorator': 'decorates',
    'annotates': 'annotates',
    # Data flow
    'assigns': 'assigns', 'returns': 'returns', 'parameter': 'parameter',
    'reads': 'reads', 'writes': 'writes',
    # References / uses
    'references': 'references', 'reference': 'references', 'ref': 'references',
    'uses': 'uses', 'use': 'uses', 'using': 'uses', 'uses_type': 'uses_type',
    # Object creation
    'creates': 'creates', 'create': 'creates', 'instantiates': 'instantiates',
    'constructs': 'creates',
    # C++ specific
    'specializes': 'specializes', 'declares': 'declares',
    'defines_body': 'defines_body', 'overrides': 'overrides',
    'hides': 'hides', 'captures': 'captures',
    # Type aliases
    'alias_of': 'alias_of', 'alias': 'alias_of',
}
...
_BRIDGE_PREPOSITIONS: frozenset = frozenset({
    'in', 'from', 'under', 'inside', 'within', 'of',
})

_INTENT_KEYWORDS: Dict[str, str] = {
    'architecture': 'architecture', 'architectural': 'architecture',
    'structure': 'architecture', 'overview': 'architecture',
    'implementation': 'implementation', 'detail': 'implementation',
    'security': 'security', 'auth': 'security', 'authentication': 'security',
    'integration': 'integration', 'integrates': 'integration',
...
class ParsedNLQuery:
    ...
    def has_structural_filters(self) -> bool:
        ...
        return bool(self.type_filter or self.path_hint or self.relationship_hint
                     or self.related_symbol)
...
class GraphQueryBuilder:
    ...
    @staticmethod
    def phrase_search(phrase: str, column: Optional[str] = None) -> str:
