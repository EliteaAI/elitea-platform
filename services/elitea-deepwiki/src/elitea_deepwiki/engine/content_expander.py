...
class ContentExpander:
    ...
    ARCHITECTURAL_SYMBOL_TYPES = {
        ...
        'restructuredtext_document', 'plaintext_document', 'documentation',
    }
    ...
    def __init__(self, graph_store=None, graph_text_index=None):
        ...
        # Optional profiling stats (enabled via DEEPWIKI_PROFILE_EXPANSION=1)
    ...
    def _build_node_index(self):
        ...
        profile = os.getenv("DEEPWIKI_PROFILE_EXPANSION") == "1"
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                ...
                for context_doc in expanded_context:
                    if (context_doc.metadata.get('replaces_original') and 
                        context_doc.metadata.get('symbol_name') == doc.metadata.get('symbol_name')):
                ...
                if replacement_doc:
                    ...
                    self.logger.debug(f"Replaced original doc with augmented version for {doc.metadata.get('symbol_name')}")
    ...
    def _expand_document_comprehensively(self, doc: Document, processed_nodes: Set[str], existing_symbols: Set[str] = None) -> List[Document]:
        """
        ...
        if symbol_type in ['class', 'interface', 'struct']:
            ...
            if symbol_type in ['struct', 'class'] and language == 'cpp':
                ...
                context_docs = self._expand_cpp_struct_comprehensively(graph_node, processed_nodes, existing_symbols, original_doc=doc)
                ...
                context_docs = self._expand_go_struct_comprehensively(graph_node, processed_nodes, existing_symbols, original_doc=doc)
    ...
    def _find_graph_node(self, symbol_name: str, file_path: str, language: str) -> Optional[str]:
        ...
        for full_name in full_name_candidates:
            if self._full_name_index and full_name in self._full_name_index:
                ...
                if profile:
                    self._profile_stats['node_index_hit_full_name'] = self._profile_stats.get('node_index_hit_full_name', 0) + 1
        ...
        for suffix_key in [symbol_name, simple_name]:
            if suffix_key and self._suffix_index and suffix_key in self._suffix_index:
                ...
                if node_id:
                    if profile:
                        self._profile_stats['node_index_hit_suffix'] = self._profile_stats.get('node_index_hit_suffix', 0) + 1
        ...
        if self.graph_text_index is not None and hasattr(self.graph_text_index, 'is_open') and self.graph_text_index.is_open:
            try:
                fts_docs = self.graph_text_index.search_smart(
                    symbol_name, k=3, intent='symbol',
                )
                if fts_docs:
                    ...
                    if best_node:
                        if profile:
                            self._profile_stats['node_index_hit_fts5'] = self._profile_stats.get('node_index_hit_fts5', 0) + 1
    ...
    def _create_document_from_graph_node(self, node_id: str, expansion_reason: str = None, 
                                          source_symbol: str = None, via_context: str = None) -> Optional[Document]:
        """
    ...
    def _filter_to_architectural_symbols(self, documents: List[Document]) -> List[Document]:
        ...
        for doc in documents:
            ...
            if symbol_type in self.ARCHITECTURAL_SYMBOL_TYPES:
                ...
                self.logger.debug(f"Filtered non-architectural: {symbol_type} | {doc.metadata.get('symbol_name', 'unknown')}")
    ...
    def _expand_class_comprehensively(self, class_node: str, processed_nodes: Set[str], existing_symbols: Set[str] = None) -> List[Document]:
        """
    ...
    def _expand_cpp_struct_comprehensively(self, struct_node: str, processed_nodes: Set[str], existing_symbols: Set[str] = None, original_doc: Document = None) -> List[Document]:
        """
        ...
        try:
            ...
            if impl_by_file:
                ...
                if struct_doc:
                    ...
                    context_docs = [d for d in context_docs 
    ...
    def _expand_go_struct_comprehensively(self, struct_node: str, processed_nodes: Set[str], existing_symbols: Set[str] = None, original_doc: Document = None) -> List[Document]:
        """
    ...
    def _augment_struct_with_go_methods(self, struct_doc: Document, methods_by_file: Dict[str, List[str]]) -> Document:
        ...
        for method_file, method_texts in sorted(methods_by_file.items()):
            ...
            augmented_content += (
                f"\n\n// Receiver methods from {method_file} "
    ...
    def _augment_cpp_function_with_implementation(self, function_node: str, function_doc: Document) -> Optional[Document]:
        """
        ...
        try:
            ...
            for source_node, target_node, edge_data in self.graph.in_edges(function_node, data=True):
    ...
    def _augment_cpp_constant_with_definition(self, constant_node: str, constant_doc: Document) -> Optional[Document]:
        """
        ...
        try:
            ...
            for other_node in candidates:
                ...
                if (other_symbol_type == 'constant' and
                    (other_name == constant_name or other_full_name == full_name) and
                    other_file and other_file != constant_file):
    ...
    def _find_struct_document(self, struct_node: str) -> Optional[Document]:
        """
    ...
    def _augment_struct_with_implementations(self, struct_doc: Document, impl_by_file: Dict[str, List[str]]) -> Document:
        """
    ...
    def _expand_method_comprehensively(self, method_node: str, processed_nodes: Set[str], original_doc: Document = None, existing_symbols: Set[str] = None) -> List[Document]:
        """
    ...
    def _expand_constructor_comprehensively(self, constructor_node: str, processed_nodes: Set[str], existing_symbols: Set[str] = None) -> List[Document]:
        """
    ...
    def _expand_constant_comprehensively(self, constant_node: str, processed_nodes: Set[str], 
                                        existing_symbols: Set[str] = None) -> List[Document]:
        """
                            ...
                            if succ_type == 'type_alias':
                                ...
                                for target_node in resolved_targets:
                                    ...
                                    if target_doc and not self._is_symbol_already_retrieved(target_doc, existing_symbols):
                                        ...
                                        self.logger.debug(f"Added constant type via alias: {target_data.get('symbol_name')}")
                                # Also include the alias itself for context
                            type_doc = self._create_document_from_graph_node(
                                succ,
                                expansion_reason='type_of',
                                source_symbol=constant_name
                            )
                    ...
                    if rel_type in ['instantiates', 'creates', 'calls']:
                        ...
                        if succ_type in ['class', 'interface']:
                            instantiated_doc = self._create_document_from_graph_node(
                                succ,
                                expansion_reason='instantiated_by',
                                source_symbol=call_data.get('symbol_name', '')
                            )
                            if instantiated_doc:
    ...
    def _expand_small_symbol_comprehensively(self, symbol_node: str, processed_nodes: Set[str], existing_symbols: Set[str] = None) -> List[Document]:
        """
    ...
    def _expand_type_definition_comprehensively(self, type_node: str, processed_nodes: Set[str], existing_symbols: Set[str] = None) -> List[Document]:
        """
    ...
    def _expand_type_alias_comprehensively(self, alias_node: str, processed_nodes: Set[str],
                                           existing_symbols: Set[str] = None) -> List[Document]:
        """
                ...
                if edge_data:
                    for edge in edge_data.values():
                        ...
                        if rel_type in ['inheritance', 'extends', 'implements', 'implementation']:
                            ...
                            expansion_reason = 'extended_by' if rel_type in ['inheritance', 'extends'] else 'implemented_by'
                            parent_doc = self._create_document_from_graph_node(
                                succ, 
                                expansion_reason=expansion_reason,
                                source_symbol=source_class_name
                            )
                            if parent_doc:
                                ...
                                grandparent_docs = check_relationships_from_node(succ, current_depth + 1, max_depth, succ_name)
            ...
            for edge in edge_data.values():
                ...
                if symbol_type == 'constructor':
                    constructor_doc = self._create_document_from_graph_node(
                        constructor_node,
                        expansion_reason='constructor_of',
                        source_symbol=class_name
                    )
                    if constructor_doc:
                        constructor_docs.append(constructor_doc)
                        processed_nodes.add(constructor_node)
                        logger.debug(f"Added constructor via DEFINES: {constructor_data.get('symbol_name', constructor_node)}")
                    break
    ...
    def _expand_class_method_parameter_types(self, class_node: str, processed_nodes: Set[str], existing_symbols: Set[str] = None) -> List[Document]:
        """
    ...
    def _find_composition_context(self, class_node: str, processed_nodes: Set[str], limit: int = 2, depth: int = 0, max_depth: int = 2) -> List[Document]:
        """
                            ...
                            if field_edge_data:
                                for field_edge in field_edge_data.values():
                                    ...
                                    if rel_type in ['composition', 'aggregation']:
                                        ...
                                        comp_type = (comp_doc.metadata.get('symbol_type', '') or comp_doc.metadata.get('type', '')).lower() if comp_doc else ''
                                        if comp_doc and comp_type in ['class', 'interface', 'struct']:
                                            ...
                                            if depth + 1 < max_depth:
                                                transitive_comps = self._find_composition_context(
                                                    field_succ, processed_nodes, limit=limit,
                ...
                for edge in edge_data.values():
                    ...
                    if rel_type in ['uses', 'composition', 'aggregation', 'creates']:
                        ...
                        if comp_doc and comp_type in ['class', 'interface', 'struct']:
                            ...
                            if depth + 1 < max_depth:
                                transitive_comps = self._find_composition_context(
                                    succ, processed_nodes, limit=limit,
                                    depth=depth + 1, max_depth=max_depth
                                )
                                composition_docs.extend(transitive_comps)
            ...
            if edge_data:
                for edge in edge_data.values():
                    ...
                    if rel_type in ['references', 'creates', 'uses']:
                        ...
                        func_type = (func_doc.metadata.get('symbol_type', '') or func_doc.metadata.get('type', '')).lower() if func_doc else ''
            ...
            if edge_data:
                for edge in edge_data.values():
                    ...
                    if rel_type in ['aggregation', 'composition', 'references', 'uses']:
                        ...
                        class_type = (class_doc.metadata.get('symbol_type', '') or class_doc.metadata.get('type', '')).lower() if class_doc else ''
    ...
    def _find_called_free_functions(self, class_node: str, processed_nodes: Set[str], 
                                    existing_symbols: Set[str] = None, limit: int = 5) -> List[Document]:
        """
                        ...
                        if rel_type in ['calls', 'invokes']:
                            ...
                            if succ_type == 'function':
                                ...
                                builtins = {
                                    ...
                                    'sorted', 'sum', 'min', 'max', 'abs', 'round', 'pow', 'all', 'any',
                                    'console.log', 'console.error', 'console.warn', 'alert', 'log'
                                }
                                ...
                                via_context = f"via {calling_method}()" if calling_method and calling_method != class_name else None
    ...
    def _find_method_callers(self, method_node: str, processed_nodes: Set[str], limit: int = 2) -> List[Document]:
        ...
        if os.getenv("DEEPWIKI_ENABLE_HEURISTIC_CALLER_SCAN") != "1":
            logger.debug("Skipping heuristic caller scan; set DEEPWIKI_ENABLE_HEURISTIC_CALLER_SCAN=1 to enable")
        ...
        for source, target, edge_data in self.graph.edges(data=True):
            ...
            if relationship_type == 'calls':
                ...
                if (source_symbol_type == 'method' and 
                    ...
                    for var_source, var_target, var_edge_data in self.graph.in_edges(target, data=True):
                        if len(caller_docs) >= limit:
                            break
    ...
    def _resolve_to_architectural_parent(self, node: str, max_depth: int = 5) -> Optional[str]:
    ...
    def _expand_return_type(self, node: str, processed_nodes: Set[str], 
                           existing_symbols: Set[str] = None) -> List[Document]:
        """
        ...
        # Skip primitives
        primitives = ['int', 'str', 'float', 'bool', 'double', 'long', 'short', 'char', 
                     'byte', 'boolean', 'string', 'number', 'any', 'object']
    ...
    def _expand_parameter_types(self, node: str, processed_nodes: Set[str],
                               existing_symbols: Set[str] = None) -> List[Document]:
        """
        ...
        # Primitives to skip
        primitives = ['int', 'str', 'float', 'bool', 'double', 'long', 'short', 'char',
                     'byte', 'boolean', 'string', 'number', 'any', 'object', 'void', 'none']
    ...
    def _expand_composed_classes_of_parameters(self, method_node: str, parameter_type_docs: List[Document],
                                               processed_nodes: Set[str], existing_symbols: Set[str] = None) -> List[Document]:
        """
        ...
        if not self.graph or not parameter_type_docs:
            self.logger.debug(f"Skipping composed class expansion: graph={self.graph is not None}, param_docs={len(parameter_type_docs) if parameter_type_docs else 0}")
            return context_docs
    ...
    def _expand_created_classes(self, node: str, processed_nodes: Set[str],
                                existing_symbols: Set[str] = None) -> List[Document]:
        """
            ...
            for _, target_node, edge_data in self.graph.out_edges(check_node, data=True):
                ...
                if class_doc:
                    ...
                    if not existing_symbols or not self._is_symbol_already_retrieved(class_doc, existing_symbols):
                        ...
                        self.logger.debug(f"Expanded to created class/function: {class_name} from {node_data.get('symbol_name')}")
                        ...
                        if target_symbol_type == 'function':
                            function_expanded = self._expand_created_classes(target_node, processed_nodes, existing_symbols)
