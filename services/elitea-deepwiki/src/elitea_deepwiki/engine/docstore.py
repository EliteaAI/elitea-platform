...
class DocstoreIndex:
    ...
    @classmethod
    def load(cls, cache_dir: Path, cache_key: str) -> Optional["DocstoreIndex"]:
...
def build_docstore_cache(
    ...
    try:
        ...
        logger.info(
            f"Saved docstore cache (docs={len(doc_ids)}) to "
            f"{docstore_path.name} / {index_path.name}"
        )
        return index_path
...
def load_docstore_cache(
    cache_dir: Path,
    cache_key: str,
) -> Optional[Tuple[MMapDocstore, Dict[int, str], List[Document]]]:
