...
HEADER_TYPES = {"graph", "flowchart", "sequenceDiagram", "classDiagram", "stateDiagram", "erDiagram", "gantt", "journey", "mindmap", "pie"}
...
def _add_sequence_participants(text: str, fixes: List[str], cfg: SanitizerConfig) -> str:
    ...
    if near_miss_map:
        ...
        msg_pat = re.compile(rf'^(?P<indent>\s*)(?P<src>{_PID})\s*(?P<arrow>-{{1,2}}>>|-->>|->>)\s*(?P<dst>{_PID})(?P<rest>\s*:.*)$')
        new_lines = []
        changed_any = False
...
RESERVED_SEQ_KEYWORDS = {  # subset of mermaid sequence keywords & control words
    'link','click','note','over','alt','loop','par','and','opt','actor','participant','end'
}
...
def _post_normalize(text: str, fixes: List[str]) -> str:
    ...
    if 'sequenceDiagram' in text:
        ...
        def _split_multi_deactivate(seq_t: str) -> str:
            ...
            changed = False
            for ln in lines:
                m = re.match(r'^\s*deactivate\s+([A-Za-z0-9_\-]+\s*,\s*[A-Za-z0-9_\-]+(?:\s*,\s*[A-Za-z0-9_\-]+)*)\s*$', ln)
                if m:
...
def sanitize_mermaid_diagram(diagram: str, cfg: Optional[SanitizerConfig] = None) -> Tuple[str, List[str], List[str]]:
    ...
    if header in FLOWCHART_LIKE or (header == "graph"):
        ...
        def _auto_nodes_for_orphan_labels(flow: str) -> str:
            ...
            orphan_re = re.compile(r'^(?P<indent>\s*)(?P<src>[A-Za-z0-9_]+)\s*-->\s*\|"(?P<edge>[^"]+)"\|\s*"(?P<label>[^"\n]+)"\s*$')
            changed = False
    ...
    if header == SEQUENCE:
        ...
        def _avoid_reserved_aliases(seq_text: str) -> str:
            ...
            for old,new in alias_map.items():
                ...
                def sender_repl(m: re.Match) -> str:
                    segment = m.group(0)
                    # case sensitive replace last occurrence of old before arrow
                    return re.sub(rf'\b{old}\b(?=-{{1,2}}>>)', new, segment)
        ...
        def _align_near_miss_aliases(seq_t: str) -> str:
            ...
            def fix_line(ln: str) -> str:
                nonlocal changed
                m = re.match(r'^(?P<indent>\s*)(?P<src>[A-Za-z0-9_\-]+)\s*(?P<arrow>-{1,2}>>|-->>|->>)\s*(?P<dst>[A-Za-z0-9_\-]+)(?P<rest>\s*:.*)$', ln)
                if not m:
                    return ln
                src = m.group('src'); dst = m.group('dst')
    ...
    if header == SEQUENCE:
        ...
        def _collapse_line_continuations(t: str) -> str:
            ...
            new_t = re.sub(r'^(?P<lhs>[A-Za-z0-9_\-]+)\s*(?P<arrow>-{1,2}>>|-->>|->>)\s*(?P<rhs>[A-Za-z0-9_\-]+):\s*(?P<msg>.+)$', msg_repl, t, flags=re.MULTILINE)
            if changed:
    ...
    text2_late = re.sub(r'(?P<id>[A-Za-z0-9_\-]+)\["(?P<inner>(?:[^"\\]|\\.(?!\]))*)"\]', _strip_quote_before_single_indexer, text2)
    if text2_late != text2:
        text2 = text2_late
    ...
    text2_final_gen = re.sub(r'(?P<id>[A-Za-z0-9_\-]+)\["(?P<inner>(?:[^"\\]|\\.(?!\]))*)"\]', _final_generics_fix, text2)
    if text2_final_gen != text2:
        text2 = text2_final_gen
    return text2, list(dict.fromkeys(fixes)), errors


def sanitize_content(content: str, cfg: Optional[SanitizerConfig] = None) -> Tuple[str, SanitizationSummary]:
    cfg = cfg or SanitizerConfig()
    ...
    for i, m in enumerate(MERMAID_FENCE_RE.finditer(content)):
        ...
        record.status = (
            "failed" if errors else ("fixed" if record.was_modified else "valid")
        )
