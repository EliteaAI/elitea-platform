...
class AgenticDocGeneratorV2:
    ...
    def __init__(
        ...
        retriever_stack: Optional[Any] = None
    ):
        self.llm = llm
        ...
        logger.info(f"AgenticDocGeneratorV2 initialized: {len(self.all_docs)} docs, "
                   f"{len(self.all_rel_paths)} unique paths")
        
        # These will be set during planning
        self.page_structure: Optional[PageStructure] = None
        self.planned_sections: List[PlannedSection] = []
        self.symbol_index: List[SymbolSummary] = []

        # Stage B selection controls
        self.use_stage_b = os.getenv("DEEPWIKI_AGENTIC_STAGE_B", "1") == "1"
        self.stage_b_trace = os.getenv("DEEPWIKI_AGENTIC_STAGE_B_TRACE", "0") == "1"
    ...
    def _maybe_drop_full_doc_cache(self, stage: str) -> None:
        ...
        drop_requested = self._get_env_int("DEEPWIKI_AGENTIC_DROP_EXPANDED_DOCS", 0) == 1
        if not drop_requested:
            return

        force_drop = self._get_env_int("DEEPWIKI_AGENTIC_DROP_EXPANDED_DOCS_FORCE", 0) == 1
        ...
        gc.collect()
        logger.info(
            f"[MEM] Dropped full doc cache at {stage}: docs={dropped_docs}, paths={dropped_paths}"
        )
    ...
    def _bind_llm_max_tokens(self, max_tokens: Optional[int]):
    ...
    def _bind_llm_max_tokens_on(self, llm: BaseLanguageModel, max_tokens: Optional[int]):
    ...
    def _build_repo_outline(self) -> str:
        ...
        max_dirs = self._get_env_int("DEEPWIKI_AGENTIC_OUTLINE_DIRS_MAX", 25)
        max_files = self._get_env_int("DEEPWIKI_AGENTIC_OUTLINE_FILES_MAX", 20)
    ...
    def _get_graph_node_id(self, doc: Document) -> Optional[str]:
    ...
    def _get_relationship_policy(self, intent: str) -> Dict[str, Any]:
        ...
        cap_per_edge = self._get_env_int("DEEPWIKI_AGENTIC_EXPAND_CAP_PER_EDGE", 3)
        max_hops = self._get_env_int("DEEPWIKI_AGENTIC_EXPAND_MAX_HOPS", 1)
    ...
    def _select_symbols_for_section(
        ...
        primary_limit = self._get_env_int("DEEPWIKI_AGENTIC_PRIMARY_LIMIT", 10)
    ...
    def _build_page_context_snippets(self) -> str:
        ...
        per_section = self._get_env_int("DEEPWIKI_AGENTIC_PAGE_CONTEXT_PER_SECTION", 4)
        max_sections = self._get_env_int("DEEPWIKI_AGENTIC_PAGE_CONTEXT_SECTIONS_MAX", 8)
    ...
    def _generate_intro_overview_conclusion(self, config: RunnableConfig) -> None:
        ...
        generate_intro = os.getenv("DEEPWIKI_AGENTIC_GENERATE_INTRO", "1") == "1"
        generate_conclusion = os.getenv("DEEPWIKI_AGENTIC_GENERATE_CONCLUSION", "0") == "1"
        generate_overview = os.getenv("DEEPWIKI_AGENTIC_GENERATE_OVERVIEW", "0") == "1"

        if not (generate_intro or generate_conclusion or generate_overview):
            return

        overview_max = self._get_env_int("DEEPWIKI_AGENTIC_OVERVIEW_SECTIONS_MAX", 1)
        max_tokens = self._get_env_int("DEEPWIKI_AGENTIC_INTRO_MAX_OUTPUT_TOKENS", 4096)
    ...
    def _populate_sections_with_retrieval(self) -> None:
        ...
        seed_k = self._get_env_int("DEEPWIKI_AGENTIC_SECTION_SEED_K", 15)
    ...
    def _build_symbol_index(
        ...
        for doc_id, doc in self.all_docs.items():
            ...
            if symbol_name:
                ...
                symbol_index.append(SymbolSummary(
                    name=symbol_name,
    ...
    def _format_symbol_index_for_prompt(self, max_symbols: Optional[int] = None) -> str:
    ...
    def _run_planning_phase(self, config: RunnableConfig) -> PageStructure:
        ...
        """
        start_time = time.time()
        trace_plan = self._get_env_int("DEEPWIKI_AGENTIC_PLAN_TRACE", 0) == 1
        logger.info(f"[PLAN] Starting planning phase for '{self.page_spec.page_name}'")
        
        max_symbol_index = self._get_env_int("DEEPWIKI_AGENTIC_SYMBOL_INDEX_MAX", 250)

        plan_mode = os.getenv("DEEPWIKI_AGENTIC_PLAN_MODE", "queries").lower()
        fast_estimate = self._get_env_int("DEEPWIKI_AGENTIC_FAST_TOKEN_ESTIMATE", 1) == 1
        ...
        if plan_mode == "symbols":
            ...
            # Format for prompt
            max_paths = self._get_env_int("DEEPWIKI_AGENTIC_PATHS_MAX", 120)
            ...
            if trace_plan:
                logger.info(
                    f"[PLAN][TRACE] Planning mode: queries (no symbol index), "
                    f"outline_chars={len(repo_outline)}"
                )
        ...
        logger.info(f"[PLAN] Invoking LLM for planning...")
        ...
        if SKIP_REPO_CONTEXT_FOR_PAGES:
            logger.info(f"⚡ SKIP_REPO_CONTEXT_FOR_PAGES=1: Omitting {len(self.repo_context or ''):,} char repository_context (agentic planning)")
        ...
        if trace_plan:
            ...
            prompt_text = "\n\n".join(
                self._normalize_llm_content_to_text(message) for message in prompt_messages
            )
            logger.info(
        ...
        plan_max_tokens = self._get_env_int("DEEPWIKI_AGENTIC_PLAN_MAX_OUTPUT_TOKENS", 16_000)
        ...
        for attempt in (1, 2):
            try:
                ...
                if trace_plan:
                    logger.info(
                        f"[PLAN][TRACE] LLM invoke: {time.perf_counter() - invoke_start:.2f}s "
                        f"(attempt={attempt})"
                    )
                raw_text = self._normalize_llm_content_to_text(llm_message)
                ...
                if os.getenv("DEEPWIKI_LOG_LLM_PAGE_PLAN_RAW") == "1":
                    raw_len = len(raw)
                    ...
                    logger.warning(
                        f"[PLAN] Planning parse/validation failed (attempt={attempt}/2): {e}. "
                ...
                logger.error(
                    f"[PLAN] Planning failed with unexpected error: {e}. Response prefix: {prefix}",
                    exc_info=True
                )
                raise
        ...
        for i, ds in enumerate(response.detail_sections):
            logger.info(f"[PLAN]   Detail {i+1}: '{ds.title}' "
    ...
    def _build_section_context(self, section: PlannedSection) -> None:
        ...
        if not docs:
            logger.warning(
                f"[GEN] No documents available for '{section.title}'. "
                "Section context will be empty."
            )
        ...
        token_counter = get_token_counter()
        section_budget = self._get_env_int(
            "DEEPWIKI_AGENTIC_SECTION_CONTEXT_TOKENS",
            self.SECTION_TOKEN_BUDGET
        )
        code_budget = self._get_env_int(
            "DEEPWIKI_AGENTIC_SECTION_CODE_TOKENS",
            int(section_budget * 0.75)
        )
        signatures_budget = self._get_env_int(
            "DEEPWIKI_AGENTIC_SIGNATURES_TOKEN_BUDGET",
            max(section_budget - code_budget, 0)
        )
        max_signatures = self._get_env_int("DEEPWIKI_AGENTIC_SIGNATURES_MAX", 80)
    ...
    def _find_doc_by_symbol(
        ...
        docs_list: Optional[List[Any]] = None
    ) -> Optional[Any]:
    ...
    def _build_system_prompt(self) -> str:
        ...
        section_list = "\n".join(
            ...
            for s in self.planned_sections
        )
    ...
    def _build_user_prompt(self, section: PlannedSection) -> str:
        ...
        return AGENTIC_USER_PROMPT.format(
            ...
            suggested_elements=", ".join(section.suggested_elements) if section.suggested_elements else "diagrams, code examples, tables",
            toc_block=toc_block,
    ...
    def _handle_tool_calls(self, response, section: PlannedSection) -> Optional[str]:
        ...
        for tool_call in tool_calls:
            ...
            if tool_name == 'commit_section':
                ...
                if requested_name:
                    valid_sections = [s.title for s in self.planned_sections 
                                     if s.title not in self.doc_state.completed_sections]
    ...
    def generate(self, config: RunnableConfig) -> str:
        ...
        while self.current_section_idx < len(self.planned_sections) and iteration < max_iterations:
            ...
            try:
                section_max_tokens = self._get_env_int(
                    "DEEPWIKI_AGENTIC_SECTION_MAX_OUTPUT_TOKENS",
                    16_000
                )
