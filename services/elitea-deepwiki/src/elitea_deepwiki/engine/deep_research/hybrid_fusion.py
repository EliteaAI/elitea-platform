...
Feature flag
~~~~~~~~~~~~
``DEEPWIKI_HYBRID_FUSION=1`` enables RRF in ``search_codebase``.
...
# Feature flag
# ---------------------------------------------------------------------------
HYBRID_FUSION_ENABLED: bool = os.environ.get("DEEPWIKI_HYBRID_FUSION", "0") == "1"
...
def fuse_search_results(
    ...
    *,
    ...
    fused = reciprocal_rank_fusion(
        ranked_lists, k=k, cap=cap, dedup_key=dedup_key,
    )
    docs: List[Document] = []
    for fr in fused:
