...
def _resource_logging_enabled() -> bool:
    return os.getenv("DEEPWIKI_RESOURCE_LOG", "0") in {"1", "true", "yes", "on"}


def _min_duration_ms() -> int:
    try:
        return int(os.getenv("DEEPWIKI_RESOURCE_LOG_MIN_DURATION_MS", "0"))
    except ValueError:
        return 0
...
def resource_monitor(phase: str, logger: Optional[logging.Logger] = None) -> Iterator[None]:
    ...
    try:
        ...
        if min_ms and wall_s * 1000 < min_ms:
            return
        ...
        msg = (
            f"[RESOURCE] {phase} "
            f"wall={wall_s:.2f}s cpu={cpu_s:.2f}s cpu%={cpu_pct:.1f}%"
        )
