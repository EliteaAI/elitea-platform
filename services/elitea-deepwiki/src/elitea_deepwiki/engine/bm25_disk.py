...
class BM25SqliteIndex:
    ...
    def load(
        ...
        try:
            ...
            rows = conn.execute(
                "SELECT doc_idx, doc_id, length FROM docs ORDER BY doc_idx"
            ).fetchall()
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        "CREATE TABLE docs (doc_idx INTEGER PRIMARY KEY, doc_id TEXT, length INTEGER)"
    )
    conn.execute(
        "CREATE TABLE postings (term TEXT, doc_idx INTEGER, tf INTEGER)"
    )
        ...
        with open(docstore_index.docstore_path, "rb") as f:
            ...
            try:
                ...
                for doc_id in docstore_index.doc_ids:
                    ...
                    postings = [
                        (term, sequential_doc_idx, int(tf))
