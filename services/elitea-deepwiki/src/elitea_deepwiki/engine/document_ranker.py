...
class DocumentRanker:
    ...
    def rank_expanded_documents(
        ...
        logger.info(f"Ranking complete: Tier1={metrics['tier1_count']}, "
    ...
    def _calculate_relationship_score(self, symbol_name: str, topic: str) -> float:
        ...
        if edges_to_topic:
            ...
            for edge_key, edge_data in edges_to_topic.items():
        ...
        if edges_from_topic:
            # MultiDiGraph returns dict of edge_key -> edge_data
            for edge_key, edge_data in edges_from_topic.items():
                rel_type = edge_data.get('relationship_type', '')
                weight = self.RELATIONSHIP_WEIGHTS.get(rel_type, 0)
                score += weight
    ...
    def _assign_tiers(
        ...
        for doc, score in scored_docs:
            ...
            if not assigned:
                logger.warning(f"Document {doc.metadata.get('symbol_name', 'unknown')} "
                             f"exceeds all tier budgets - skipping")
