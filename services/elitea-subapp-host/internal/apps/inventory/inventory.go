...
// copy of conformance/provider/fixtures/inventory/descriptor/legacy-v0/
...
//
// There is NO ENGINE YET. The analysis engine — the legacy inventory_plugin's
// knowledge-graph layer — reaches this host as a sidecar in stage I3; until
// then a host serving this application runs the unavailable runner (the
// default), so /descriptor and /health answer and every tool refuses in
// band with a reason a caller can read.
package inventory
	...
	Families: []spi.Family{
		{
			...
			Tools: []string{
				...
				"get_cache_stats", "cleanup_cache", "get_sources_status", "normalize_types",
				"rebuild_indices", "get_type_stats", "link_toolkits_to_tools", "connect_orphan_nodes",
