...
// cancellingRouter models bifrost's real cancellation behaviour: when the
// provider context is cancelled it emits a BifrostError chunk carrying
// BilledUsage — the provider's count of tokens it had already processed — and
// then closes. See core@v1.7.3 providers/utils/utils.go: "Bill for tokens the
// provider already processed before the client disconnected".
...
func (r *cancellingRouter) ChatCompletionStreamRequest(ctx *schemas.BifrostContext, _ *schemas.BifrostChatRequest) (chan *schemas.BifrostStreamChunk, *schemas.BifrostError) {
	ch := make(chan *schemas.BifrostStreamChunk)
	go func() {
		defer close(ch)
		for _, c := range r.preamble {
		...
		cancelChunk.BifrostError.ExtraFields.BilledUsage = &schemas.BifrostLLMUsage{
			PromptTokens: r.billedIn, CompletionTokens: r.billedOut,
		}
		// The drain is still reading, so this send lands even post-cancel.
		select {
		case ch <- cancelChunk:
		case <-time.After(time.Second):
		}
...
// TestCutStream_BillsProviderAccumulatedUsage: when the grace expires and we
// cut the provider stream ourselves, bifrost hands back the usage the provider
// had already accrued. That is a provider-reported measure of work actually
// done — not a gateway estimate — so it must be billed rather than discarded
// (human decision 2026-08-05).
func TestCutStream_BillsProviderAccumulatedUsage(t *testing.T) {
	gate := allowGate()
	events := newRecordingEvents()
	calc := &fakeCostEstimator{inputRateNano: gateInputRateNano, outputRateNano: gateOutputRateNano}
	...
	if got := gate.updateCalls.Load(); got != 1 {
		t.Fatalf("UpdateUsage calls = %d, want 1 — the provider reported %d/%d tokens of work it had "+
			"already done; discarding that is throwing away real spend",
			got, router.billedIn, router.billedOut)
	}
	if got := gate.getLastUpdateCostNano(); got != gateWantCostNano {
		t.Errorf("billed %d nano-USD, want %d", got, gateWantCostNano)
	}
	...
	defer events.mu.Unlock()
	if len(events.events) != 0 {
		t.Errorf("published %d loss event(s) for a stream that WAS billed; want 0", len(events.events))
	}
}

// TestCutStream_TerminalTrailerBeatsAccumulated pins the precedence: the final
// trailer is the authoritative number, and a later accumulated count must never
// overwrite it downward.
func TestCutStream_TerminalTrailerBeatsAccumulated(t *testing.T) {
	gate := allowGate()
	calc := &fakeCostEstimator{inputRateNano: gateInputRateNano, outputRateNano: gateOutputRateNano}
	h := NewHandler(&fakeRouter{}, nil, nil, WithBudgetGate(gate, calc))
	...
	s := h.newChatSettler("streamOpenAI", ctx, sc, "openai", "gpt-4o", nil)
	s.observe(chatTrailer(11, 22)) // authoritative final usage
	// A cancellation chunk arriving afterwards with a SMALLER accrued count.
	cancelChunk := &schemas.BifrostStreamChunk{BifrostError: &schemas.BifrostError{
		Error: &schemas.ErrorField{Message: "cancelled"},
	}}
	cancelChunk.BifrostError.ExtraFields.BilledUsage = &schemas.BifrostLLMUsage{
		PromptTokens: 1, CompletionTokens: 1,
	}
	s.observe(cancelChunk)
	s.report(lossReasonWriteError, drainOutcomeGraceExpired)
	h.DrainBilling()

	if got := gate.getLastUpdateCostNano(); got != gateWantCostNano {
		t.Errorf("billed %d nano-USD, want %d — the terminal trailer must win over a later "+
			"accumulated count", got, gateWantCostNano)
	}
}

// TestCutStream_NoAccumulatedUsageStillMetersTheLoss: bifrost omits BilledUsage
// when nothing measurable was consumed, and that case must still surface as a
// loss rather than a zero-token bill.
func TestCutStream_NoAccumulatedUsageStillMetersTheLoss(t *testing.T) {
	gate := allowGate()
	events := newRecordingEvents()
	router := &cancellingRouter{
		preamble:  []*schemas.BifrostStreamChunk{chatDelta("c1", "hello "), chatDelta("c2", "world")},
	...
	if got := gate.updateCalls.Load(); got != 0 {
		t.Fatalf("UpdateUsage calls = %d, want 0 (nothing measurable was consumed)", got)
	}
	if p := events.decodeUnbilled(t); p.Reason != lossReasonWriteError {
