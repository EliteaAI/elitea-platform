...
test('J16: create pipeline, edit flow graph, save', async ({ page }) => {
  ...
  const createButton = page
    .getByRole('button', { name: /create|new pipeline/i })
  ...
  const flowCanvas = page
    .locator('.react-flow')
    .or(page.getByTestId('edit-pipeline-configuration-tab-panel'));
  ...
  await expect(flowCanvas.or(page.getByTestId('edit-pipeline-configuration-form-gap'))).toBeVisible({
    timeout: 10_000,
  });
