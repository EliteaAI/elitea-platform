...
import { codeBlockPlaceholder, genericTokenText, ordinalFor, stripEmoji } from './ttsShared.helpers';
...
export function blockText(tokens: readonly Token[] | undefined): string {
  ...
  return tokens.map((token) => (BLOCK_HANDLERS[token.type] ?? genericTokenText)(token)).join('');
}
