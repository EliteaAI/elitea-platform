 ...
 *
 * Local duplicate of `features/agents/lib/useFieldFocus.ts` (byte-for-byte
 * identical) — `no-sideways-features` forbids `features/toolkits` importing
 * `features/agents`, and this plain, dependency-free hook has no promoted
 * `shared/`/`entities/` home (it isn't one of the 4 "not promoted, duplicate
 * locally" hooks named in the Wave-2 brief, nor part of the
 * `entities/application-form`/`entities/toolkit` promotion). Tracks which of
 * the Name/Description fields in `NameDescriptionInput.tsx` currently has
 * focus, to show the "N characters left" hint only for the focused field.
 */
