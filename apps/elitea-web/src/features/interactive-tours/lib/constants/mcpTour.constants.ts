/**
 * Port of `apps/elitea-ui/src/[fsd]/features/interactive-tours/lib/constants/mcpTour.constants.js`
 */

import type { TourStep } from '../types';

import { SHARED_TOUR_TARGETS } from './interactiveTour.constants';
import { MCP_TOUR_TARGETS } from './mcpTourTargets.constants';

/**
 * RouteDefinitions.CreateMCP — the old source checked whether the current
 * pathname includes the MCP creation page to skip the connection-status step.
 * In the new TanStack-Router app the path is `/mcps/create/$mcpType`.
 */
const CREATE_MCP_PATH = '/mcps/create';

const shouldSkipConnectionStatusStep = (): boolean => {
  if (typeof window === 'undefined') return false;

  return window.location.pathname.includes(CREATE_MCP_PATH);
};
...
export const mcpTourSteps: TourStep[] = [
  {
    id: 'what-are-mcps',
