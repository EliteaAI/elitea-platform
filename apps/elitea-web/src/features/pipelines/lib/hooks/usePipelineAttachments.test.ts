...
describe('usePipelineAttachments', () => {
  it('disableAttachments is true when internal_tools omits "attachments"', () => {
    const { result } = renderHook(() => usePipelineAttachments({ internalTools: ['internal_mcp'], versionId: 1 }));
  ...
  it('disableAttachments is false when internal_tools includes "attachments"', () => {
    const { result } = renderHook(() =>
      usePipelineAttachments({ internalTools: ['internal_mcp', 'attachments'], versionId: 1 }),
    );
    expect(result.current.disableAttachments).toBe(false);
  });

  it('adds and deletes attachments when enabled', () => {
    const { result } = renderHook(() => usePipelineAttachments({ internalTools: ['attachments'], versionId: 1 }));
  ...
  it('clears attachments as soon as attachments become disabled', () => {
    const { result, rerender } = renderHook(
      ({ internalTools }: { internalTools: readonly string[] }) => usePipelineAttachments({ internalTools, versionId: 1 }),
  ...
  it('clears attachments when versionId changes', () => {
    const { result, rerender } = renderHook(
      ({ versionId }: { versionId: number }) => usePipelineAttachments({ internalTools: ['attachments'], versionId }),
