...
import { usePipelineMCPToolsStatusMonitor } from './usePipelineMCPToolsStatusMonitor';
...
describe('usePipelineMCPToolsStatusMonitor', () => {
  it('applies an mcp_status event to the matching tool when the version has an MCP tool', () => {
    ...
    renderHook(
      () => usePipelineMCPToolsStatusMonitor({ tools: [REGULAR_TOOL, MCP_TOOL], projectId: 'proj-1', onToolsChange }),
  ...
  it('does nothing when the version has no MCP-flavoured tool (isMCP short-circuit)', () => {
    const client = createTestSocketClient();
    const onToolsChange = vi.fn();
    renderHook(() => usePipelineMCPToolsStatusMonitor({ tools: [REGULAR_TOOL], projectId: 'proj-1', onToolsChange }), {
      wrapper: withSocket(client),
    });
  ...
  it('ignores an event for a different project', () => {
    ...
    const onToolsChange = vi.fn();
    renderHook(
      () => usePipelineMCPToolsStatusMonitor({ tools: [MCP_TOOL], projectId: 'proj-1', onToolsChange }),
      { wrapper: withSocket(client) },
    );
  ...
  it('applies the event when projectId is undefined (no gating possible yet)', () => {
    ...
    const onToolsChange = vi.fn();
    renderHook(
      () => usePipelineMCPToolsStatusMonitor({ tools: [MCP_TOOL], projectId: undefined, onToolsChange }),
      { wrapper: withSocket(client) },
    );
