...
import type { ChatConfig, PredictResponse } from "../model";

export const getWebchatSyncResponseMock = (
...
export const getGetChatConfigResponseMock = (
  overrideResponse: Partial<Extract<ChatConfig, object>> = {},
): ChatConfig => ({
  models: Array.from(
    { length: faker.number.int({ min: 1, max: 10 }) },
    (_, i) => i + 1,
    ...
    name: faker.string.alpha({ length: { min: 10, max: 20 } }),
    config_type: faker.string.alpha({ length: { min: 10, max: 20 } }),
  })),
  default_model: faker.string.alpha({ length: { min: 10, max: 20 } }),
  ...overrideResponse,
});

export const getWebchatSyncMockHandler = (
  overrideResponse?:
    | PredictResponse
    | ((
...
export const getGetChatConfigMockHandler = (
  overrideResponse?:
    | ChatConfig
    | ((
        info: Parameters<Parameters<typeof http.get>[1]>[0],
      ) => Promise<ChatConfig> | ChatConfig),
  options?: RequestHandlerOptions,
) => {
  return http.get(
    "*/elitea_core/chat_config/prompt_lib/:projectId",
...
export const getChatMock = () => [
  getWebchatSyncMockHandler(),
  getGetChatConfigMockHandler(),
];
