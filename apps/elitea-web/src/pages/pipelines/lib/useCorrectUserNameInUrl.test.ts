...
beforeEach(() => {
  window.history.replaceState(null, '', '/pipelines/latest/42');
});
...
describe('useCorrectUserNameInUrl', () => {
  ...
  it('sets ?name= to the real name when absent from the URL', () => {
    renderHook(() => useCorrectUserNameInUrl('My Pipeline'));
    expect(new URL(window.location.href).searchParams.get('name')).toBe('My Pipeline');
  });

  it('is a no-op (does not touch history) when the URL already matches', () => {
    window.history.replaceState(null, '', '/pipelines/latest/42?name=My+Pipeline');
