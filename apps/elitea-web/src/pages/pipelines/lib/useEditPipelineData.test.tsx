...
import { useEditPipelineData } from './useEditPipelineData';
...
function detail() {
  return {
    ...
    versions: [
      ...
      { id: '2', name: 'v2', status: 'published', agent_type: 'pipeline', created_at: '2026-01-02T00:00:00Z' },
    ],
...
describe('useEditPipelineData', () => {
  ...
  it('does not fetch anything while applicationId is undefined', () => {
    const { result } = renderHook(() => useEditPipelineData('9', undefined, undefined), { wrapper });
  ...
  it("fetches the explicit version when the URL :version differs from the detail response's default version", async () => {
    ...
    const { result } = renderHook(() => useEditPipelineData('9', 42, '2'), { wrapper });
  ...
  it("does NOT issue a second fetch when the URL :version matches the detail response's own default version", async () => {
    server.use(getGetApplicationMockHandler(detail()));
    const { result } = renderHook(() => useEditPipelineData('9', 42, '1'), { wrapper });
