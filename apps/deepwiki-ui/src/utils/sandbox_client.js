...
class SandboxClient {
    constructor(config) {
        ...
        // NO X-SECRET, and no Authorization when there is no token.
        //
        // The legacy shared-string header is retired (ADR-0022 decision 6):
        // nothing in the ported service sends or honours it, and the platform
        // never read it either — a client that kept sending it was sending the
        // literal word "secret" to something that ignored it.
        //
        // `Bearer ` with an empty token is worse than no header at all: it is
        // a malformed credential that some servers reject outright rather than
        // treating the request as unauthenticated, so the session cookie never
        // gets its turn. This bundle is served same-origin behind an
        // authenticated handler, so the cookie IS the credential.
        this.headers = {
     ...
     */
    /**
     * The credential headers, without Content-Type.
     *
     * FormData uploads must not carry an explicit Content-Type — the browser
     * sets it with the multipart boundary — but they must carry everything
     * else this client authenticates with.
     */
    _credentialHeaders() {
    ...
    async createArtifact(bucketName, artifactName, artifactData) {
        ...
        const response = await this._fetch(url, {
            ...
            body: formData,
            headers: {
                // Whatever credential headers the client holds, and no
                // X-SECRET. Spread rather than named, so a header added above
                // reaches the upload too — naming them one by one is how an
                // upload ends up unauthenticated while every other call works.
                ...this._credentialHeaders()
