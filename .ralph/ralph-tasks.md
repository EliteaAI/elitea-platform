...
- [ ] BF0.8b Audit + fix 429→402 budget call sites in `services/elitea-main`
  - [ ] Find every site that treats budget/quota exhaustion as 429 and make it also accept/emit 402 (`type=budget_exceeded, code=insufficient_quota`), per spec §4.1
  - [ ] Run `cutover-ctl budget-status-audit --paths services/elitea-main` — must exit 0

- [ ] BF0.8c Audit + fix 429→402 budget call sites in `elitea-sdk`
  - [ ] Update the SDK's budget/429 handling so a 402 is treated identically to a 429-on-budget (retry/raise semantics unchanged)
  - [ ] Run `cutover-ctl budget-status-audit --paths services/elitea-main,elitea-sdk` — must exit 0 (the BF0.8 gate)
